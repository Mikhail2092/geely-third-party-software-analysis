...
// hook_geely_map_overlay_widget.js
var require_hook_geely_map_overlay_widget = __commonJS({
  "hook_geely_map_overlay_widget.js"() {
    init_node_globals();
    ...
    frida_java_bridge_default.performNow(function() {
      const TAG = "[AutoDisplayHook]";
      const LOC_SOCKET_HOST = "127.0.0.1";
      const LOC_SOCKET_PORT = 23791;
      const ROUTE_EXPORT_OFFSET = 40063332;
      const PROTOCOL_VERSION = 1;
      const PROVIDER = "geely_xingrui_l";
      const VEHICLE_PROFILE = "geely_xingrui_l";
      const MAP_PACKAGE = "com.geely.map";
      const HOOK_VERSION = "2026.07.07.auto-heartbeat";
      ...
      function androidLog(message) {
        try {
          ...
          Log.i("AutoDisplayHook", String(message));
        } catch (e) {
        }
      }
      function writeRoutePayload(payload, source) {
        const jsonText = JSON.stringify(payload);
        try {
          if (payload.routes && payload.routes.length > 0 && payload.pointCount > 1) {
            keep.lastRoutePayload = payload;
            keep.lastRoutePayloadSignature = routePayloadSignature(payload);
          }
          if (typeof keep.sendSocketLine === "function") {
            const sent = keep.sendSocketLine("ROUTE|" + jsonText + "\n", source + " route");
            if (sent) {
              log(source + " sent route points=" + payload.pointCount + ", navigating=" + payload.navigating);
            }
            return sent;
          }
          log(source + " route socket not ready");
          return false;
        } catch (e) {
          log(source + " send route failed: " + e);
          return false;
        }
      }
      function restoreRouteCache() {
        if (keep.lastRoutePayload && keep.lastRoutePayload.routes && keep.lastRoutePayload.routes.length > 0) {
          return keep.lastRoutePayload;
        }
        return null;
      }
      function routePayloadSignature(payload) {
        try {
          if (!payload || !payload.routes || payload.routes.length === 0) {
            return "";
          }
          const route = payload.routes.find(function(item) {
            return item && item.selected;
          }) || payload.routes[Math.max(0, Math.min(payload.selectIndex || 0, payload.routes.length - 1))] || payload.routes[0];
          let sample = "";
          try {
            ...
            sample = [first, middle, last].map(function(point) {
      ...
      function cloneNativeRoutePayloadWithSelection(selectIndex, source) {
        try {
          const cached = keep.nativeRoutePayload || restoreRouteCache();
          if (!cached || !cached.routes || cached.routes.length === 0) {
            return null;
          }
          const payload = JSON.parse(JSON.stringify(cached));
          const count = payload.routes.length;
          const selectedIndex = Math.max(0, Math.min(Math.floor(Number(selectIndex) || 0), count - 1));
          payload.ts = Date.now();
          payload.source = source;
          payload.navigating = true;
          payload.selectIndex = selectedIndex;
          payload.routeCount = count;
          let pointCount = 0;
          for (let i = 0; i < count; i++) {
            payload.routes[i].selected = i === selectedIndex;
            pointCount += payload.routes[i].points ? payload.routes[i].points.length : 0;
          }
          payload.pointCount = pointCount;
          return payload;
        } catch (e) {
          log(source + " clone native route cache failed: " + (e && e.stack ? e.stack : e));
          return null;
        }
      }
      function sendCachedNativeRouteByIndex(index, source) {
        const payload = cloneNativeRoutePayloadWithSelection(index, source);
        if (payload === null) {
          log(source + " skip: native route cache empty");
          return false;
        }
        return writeRoutePayload(payload, source);
      }
      function sendCachedNativeRouteByPathId(pathId, source) {
        try {
          const cached = keep.nativeRoutePayload || restoreRouteCache();
          const routes = cached && cached.routes ? cached.routes : [];
          const target = String(pathId);
          for (let i = 0; i < routes.length; i++) {
            if (String(routes[i].pathId || "") === target || String(routes[i].naviId || "") === target) {
              return sendCachedNativeRouteByIndex(i, source + ":pathIdMatched");
            }
          }
          log(source + " skip: pathId=" + target + " not in native route cache");
          return false;
        } catch (e) {
          log(source + " cached pathId route failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function sendProtocolLine(prefix, payload, source) {
        try {
          if (typeof keep.sendSocketLine !== "function") {
            log(source + " socket not ready");
            return false;
          }
          return keep.sendSocketLine(prefix + "|" + JSON.stringify(payload) + "\n", source);
        } catch (e) {
          log(source + " send failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function enqueueTask(label, task) {
        if (!keep.pendingTasks) {
          keep.pendingTasks = [];
        }
        keep.pendingTasks.push({ label, task });
        if (keep.pendingTasks.length > 120) {
          keep.pendingTasks.splice(0, keep.pendingTasks.length - 120);
        }
      }
      function runPendingTasks(maxTasks) {
        if (keep.processingTasks || !keep.pendingTasks || keep.pendingTasks.length === 0) {
          return;
        }
        keep.processingTasks = true;
        try {
          const count = Math.min(maxTasks || 8, keep.pendingTasks.length);
          for (let i = 0; i < count; i++) {
            const item = keep.pendingTasks.shift();
            ...
            try {
              ...
              log(item.label + " async task failed: " + (e && e.stack ? e.stack : e));
            }
          }
        } finally {
          keep.processingTasks = false;
        }
      }
      function retainJavaObject(obj) {
        try {
          return obj ? frida_java_bridge_default.retain(obj) : obj;
        } catch (e) {
          return obj;
        }
      }
      function objectField(obj, name) {
        try {
        ...
        try {
          if (obj !== null && obj !== void 0 && obj.getClass) {
            const field = obj.getClass().getField(name);
            if (field) {
      ...
      function numberField(obj, name, fallback) {
        try {
          const raw = objectField(obj, name);
          if (raw !== null && raw !== void 0) {
            let value = Number(raw);
            if (!Number.isFinite(value) && raw.intValue) {
              value = Number(raw.intValue());
            }
            if (!Number.isFinite(value) && raw.longValue) {
      ...
      function stringField(obj, name, fallback) {
        try {
          const raw = objectField(obj, name);
          if (raw === null || raw === void 0) {
            return fallback;
      ...
      function listSize(list) {
        try {
          return list === null || list === void 0 ? 0 : Number(list.size());
        } catch (e) {
          return 0;
        }
      }
      function listToNumberArray(list, maxCount) {
        const values = [];
        const count = Math.min(listSize(list), maxCount || 32);
        for (let i = 0; i < count; i++) {
          try {
            const item = list.get(i);
            let value = Number(item);
            if (!Number.isFinite(value) && item && item.intValue) {
      ...
      function numberArrayField(obj, name, maxCount) {
        return listToNumberArray(objectField(obj, name), maxCount);
      }
      function hasUsableLaneValues(values) {
      ...
      function readListItem(list, index) {
        try {
          if (!list || index < 0 || index >= listSize(list)) {
            return null;
          }
      ...
      function sendGuideClear(source) {
        keep.lastGuideSignature = "";
        sendProtocolLine("GUIDE", {
      ...
      function sendLaneClear(source) {
        keep.lastLaneSignature = "";
        sendProtocolLine("LANE", {
          protocolVersion: PROTOCOL_VERSION,
          messageType: "lane",
          provider: PROVIDER,
          vehicleProfile: VEHICLE_PROFILE,
      ...
      function sendTrafficLightClear(source) {
        keep.lastTrafficLightSignature = "";
        sendProtocolLine("TL", {
          protocolVersion: PROTOCOL_VERSION,
          ...
          waitRound: 0,
          clear: true
        }, source + " traffic-light-clear");
      }
      function guidePanelScore(panel, preferred) {
        if (!panel) {
          return -1;
        }
        const maneuverId = Math.floor(numberField(panel, "maneuverID", -1));
        const segmentRemain = objectField(panel, "segmentRemain");
        const segmentDistanceMeters = Math.floor(numberField(segmentRemain, "dist", -1));
        const nextRoadName = stringField(panel, "nextRouteName", "");
        let score = preferred ? 2 : 0;
        if (maneuverId >= 0) {
          score += 8;
        }
      ...
      function firstCrossInfo(naviInfo) {
        return readListItem(objectField(naviInfo, "nextCrossInfo"), 0);
      }
      function readBestGuideContext(naviInfo) {
        const panels = objectField(naviInfo, "NaviInfoData");
        const panelCount = listSize(panels);
        const flagIndex = Math.floor(numberField(naviInfo, "NaviInfoFlag", -1));
        let bestPanel = null;
        let bestPanelIndex = -1;
        let bestScore = -1;
        function tryPanel(index, preferred) {
          const panel = readListItem(panels, index);
          const score = guidePanelScore(panel, preferred);
          if (score > bestScore) {
            bestScore = score;
            bestPanel = panel;
            bestPanelIndex = index;
          }
        }
        tryPanel(flagIndex, true);
        for (let i = 0; i < panelCount; i++) {
          if (i !== flagIndex) {
            tryPanel(i, false);
          }
        }
        const routeRemain = objectField(naviInfo, "routeRemain");
        const routeDistanceMeters = Math.floor(numberField(routeRemain, "dist", -1));
        const routeTimeSeconds = Math.floor(numberField(routeRemain, "time", -1));
        let score = bestScore;
      ...
      function sendGuideFromNaviInfoList(naviInfoList, source, nextRoadOverride) {
        try {
          const count = listSize(naviInfoList);
          if (count <= 0) {
            sendGuideClear(source + ":empty");
            return false;
          }
          let bestContext = null;
          for (let i = 0; i < count; i++) {
            const item = readListItem(naviInfoList, i);
            if (!item) {
              continue;
            }
            const context = readBestGuideContext(item);
            if (!bestContext || context.score > bestContext.score) {
              bestContext = context;
            }
          }
          if (!bestContext || !bestContext.naviInfo) {
            sendGuideClear(source + ":null");
            return false;
          }
          const naviInfo = bestContext.naviInfo;
          const panel = bestContext.panel;
          const cross = bestContext.cross;
          let maneuverId = Math.floor(numberField(panel, "maneuverID", -1));
          if (maneuverId < 0) {
            maneuverId = Math.floor(numberField(cross, "maneuverID", numberField(naviInfo, "crossManeuverID", -1)));
          }
          const crossManeuverId = Math.floor(numberField(cross, "crossManeuverID", numberField(naviInfo, "crossManeuverID", maneuverId)));
          const roundaboutExit = Math.max(0, Math.floor(numberField(naviInfo, "ringOutCnt", numberField(cross, "outCnt", 0))));
          const segmentRemain = objectField(panel, "segmentRemain");
          const routeRemain = objectField(naviInfo, "routeRemain");
          let rawSegmentDistanceMeters = Math.floor(numberField(segmentRemain, "dist", -1));
          if (rawSegmentDistanceMeters < 0) {
            rawSegmentDistanceMeters = Math.floor(numberField(cross, "curToSegmentDist", numberField(naviInfo, "linkRemainDist", -1)));
          }
          const segmentDistanceMeters = keep.finalGuideDistanceExportInstalled ? -1 : rawSegmentDistanceMeters;
          const routeDistanceMeters = Math.floor(numberField(routeRemain, "dist", -1));
          const routeTimeSeconds = Math.floor(numberField(routeRemain, "time", -1));
          let nextRoadName = stringField(panel, "nextRouteName", "");
          if (nextRoadName.length <= 0) {
            nextRoadName = stringField(cross, "nextRoadName", "");
          }
          const overrideRoad = nextRoadOverride === null || nextRoadOverride === void 0 ? "" : String(nextRoadOverride).trim();
          if (overrideRoad.length > 0 && overrideRoad.toLowerCase() !== "null") {
            nextRoadName = overrideRoad;
          }
          const curRouteName = stringField(naviInfo, "curRouteName", "");
          const curSegIdx = Math.floor(numberField(naviInfo, "curSegIdx", -1));
          const curLinkIdx = Math.floor(numberField(naviInfo, "curLinkIdx", -1));
          const curPointIdx = Math.floor(numberField(naviInfo, "curPointIdx", -1));
          const signature = [
            maneuverId,
            crossManeuverId,
            roundaboutExit,
          ...
          const now = Date.now();
          if (signature === keep.lastGuideSignature && now - (keep.lastGuideSentAt || 0) < 500) {
            return true;
          }
          keep.lastGuideSignature = signature;
          keep.lastGuideSentAt = now;
          sendProtocolLine("GUIDE", {
            protocolVersion: PROTOCOL_VERSION,
            messageType: "guide",
            provider: PROVIDER,
            ...
            maneuverId,
            roundaboutExit,
            legacyIcon: maneuverId,
            maneuverInfoId: crossManeuverId,
            nextRoadName,
            segmentDistanceMeters,
            routeDistanceMeters,
            routeTimeSeconds,
            routeRemainLightCount: Math.floor(numberField(naviInfo, "routeRemainLightCount", -1)),
            rawSegmentDistanceMeters,
            curRouteName,
            curSegIdx,
            curLinkIdx,
            curPointIdx,
            guideListCount: count,
            guidePanelCount: bestContext.panelCount,
            guidePanelIndex: bestContext.panelIndex,
            guideFlagIndex: bestContext.flagIndex,
            guideScore: bestContext.score
          }, source + " guide");
          log(source + " guide maneuver=" + maneuverId + " next=" + nextRoadName + " segmentDist=" + segmentDistanceMeters + " rawSegmentDist=" + rawSegmentDistanceMeters + " routeDist=" + routeDistanceMeters + " routeTime=" + routeTimeSeconds);
          return true;
        } catch (e) {
          log(source + " guide export failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function sendFinalSegmentDistance(distanceMeters, totalDistanceMeters, source) {
        try {
          const distance = Math.floor(Number(distanceMeters));
          if (!Number.isFinite(distance) || distance < 0) {
            return false;
          }
          const totalDistance = Math.floor(Number(totalDistanceMeters));
          const now = Date.now();
          const signature = [distance, Number.isFinite(totalDistance) ? totalDistance : -1].join(":");
          if (signature === keep.lastFinalSegmentDistanceSignature && now - (keep.lastFinalSegmentDistanceSentAt || 0) < 250) {
            return true;
          }
          keep.lastFinalSegmentDistanceSignature = signature;
          keep.lastFinalSegmentDistanceSentAt = now;
          return sendProtocolLine("GUIDE", {
            protocolVersion: PROTOCOL_VERSION,
            messageType: "guide",
            provider: PROVIDER,
            vehicleProfile: VEHICLE_PROFILE,
            ...
            totalSegmentDistanceMeters: Number.isFinite(totalDistance) ? totalDistance : -1,
            finalSegmentDistance: true
          }, source + " final-segment-distance");
        } catch (e) {
          log(source + " final segment distance export failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function jsonObjectFromString(text) {
        try {
      ...
      function jsonNumber(obj, name, fallback) {
        try {
          if (!obj || obj[name] === null || obj[name] === void 0 || obj[name] === "") {
            return fallback;
          }
      ...
      function jsonString(obj, name, fallback) {
        try {
          if (!obj || obj[name] === null || obj[name] === void 0) {
            return fallback;
          }
          const value = String(obj[name]).trim();
          return value.length > 0 && value.toLowerCase() !== "null" ? value : fallback;
        } catch (e) {
          return fallback;
        }
      }
      function sendGuideFromProtocolJson(text, source) {
        try {
          const data = jsonObjectFromString(text);
          if (!data) {
            return false;
          }
          const maneuverId = Math.floor(jsonNumber(data, "icon", jsonNumber(data, "newIcon", -1)));
          const roundaboutExit = Math.max(0, Math.floor(jsonNumber(data, "roundAboutNum", 0)));
          const nextRoadName = jsonString(data, "nextRoadName", "");
          const segmentDistanceMeters = Math.floor(jsonNumber(data, "segRemainDis", jsonNumber(data, "nextSegRemainDis", -1)));
          const routeDistanceMeters = Math.floor(jsonNumber(data, "routeRemainDis", -1));
          const routeTimeSeconds = Math.floor(jsonNumber(data, "routeRemainTime", -1));
          const signature = [
            "protocol",
            maneuverId,
            roundaboutExit,
            ...
            routeDistanceMeters,
            routeTimeSeconds,
            jsonNumber(data, "curSegNum", -1),
            jsonNumber(data, "curPointNum", -1)
          ].join("|");
          const now = Date.now();
          if (signature === keep.lastGuideSignature && now - (keep.lastGuideSentAt || 0) < 500) {
            return true;
          ...
          sendProtocolLine("GUIDE", {
            ...
            maneuverId,
            roundaboutExit,
            legacyIcon: Math.floor(jsonNumber(data, "newIcon", maneuverId)),
            maneuverInfoId: maneuverId,
            nextRoadName,
            segmentDistanceMeters,
            routeDistanceMeters,
            routeTimeSeconds,
            routeRemainLightCount: Math.floor(jsonNumber(data, "routeRemainTrafficLightNum", -1)),
            curRouteName: jsonString(data, "curRoadName", ""),
            curSegIdx: Math.floor(jsonNumber(data, "curSegNum", -1)),
            curLinkIdx: -1,
            curPointIdx: Math.floor(jsonNumber(data, "curPointNum", -1)),
            etaText: jsonString(data, "etaText", ""),
            guideProtocol: true
          }, source + " guide-protocol");
          log(source + " guide-protocol maneuver=" + maneuverId + " next=" + nextRoadName + " segmentDist=" + segmentDistanceMeters + " routeDist=" + routeDistanceMeters + " routeTime=" + routeTimeSeconds);
          return true;
        } catch (e) {
          log(source + " guide protocol export failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function sendLaneFromInfo(laneInfo, source) {
        try {
          if (!laneInfo) {
            sendLaneClear(source + ":null");
            return false;
          }
          const frontLane = numberArrayField(laneInfo, "frontLane", 24);
          const backLane = numberArrayField(laneInfo, "backLane", 24);
          let extensionLane = numberArrayField(laneInfo, "extensionLane", 24);
          if (extensionLane.length <= 0) {
            extensionLane = numberArrayField(laneInfo, "backExtenLane", 24);
          }
          const optimalLane = numberArrayField(laneInfo, "optimalLane", 24);
          const count = Math.max(frontLane.length, backLane.length);
          if (count <= 0 || !hasUsableLaneValues(backLane)) {
            sendLaneClear(source + ":empty");
            return false;
          }
          const hasExplicitOptimal = optimalLane.length > 0 && hasUsableLaneValues(optimalLane);
          const recommend = [];
          for (let i = 0; i < count; i++) {
            const optimal = i < optimalLane.length ? optimalLane[i] : 255;
            const front = i < frontLane.length ? frontLane[i] : 255;
            recommend.push(hasExplicitOptimal ? optimal >= 0 && optimal !== 255 : front >= 0 && front !== 255);
          }
          const signature = [
            frontLane.join(","),
            backLane.join(","),
            recommend.map(function(v) {
              return v ? 1 : 0;
            }).join(","),
            extensionLane.join(",")
          ].join("|");
          const now = Date.now();
          if (signature === keep.lastLaneSignature && now - (keep.lastLaneSentAt || 0) < 500) {
            return true;
          }
          keep.lastLaneSignature = signature;
          keep.lastLaneSentAt = now;
          sendProtocolLine("LANE", {
            protocolVersion: PROTOCOL_VERSION,
            messageType: "lane",
            provider: PROVIDER,
          ...
          log(source + " lane count=" + count + " front=" + frontLane.join(",") + " back=" + backLane.join(","));
          return true;
        } catch (e) {
          log(source + " lane export failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function mapTrafficLightDirection(rawDirection) {
        const value = Math.floor(Number(rawDirection));
      ...
      function mapTrafficLightStatus(rawLightType) {
        const value = Math.floor(Number(rawLightType));
        if (value === 2) {
          return 1;
        }
        if (value === 3) {
      ...
      function mapProtocolTrafficLightStatus(rawStatus) {
        const value = Math.floor(Number(rawStatus));
        if (value === 1 || value === 2) {
          return 1;
        }
        if (value === 4 || value === 5) {
      ...
      function currentNaviPathId() {
        try {
          const ModuleServiceUtils = frida_java_bridge_default.use("com.autosdk.common.utils.ModuleServiceUtils");
          const driveService = ModuleServiceUtils.getDriveService();
          if (!driveService) {
            return -1;
          }
          const pathInfo = driveService.getNaviPathInfo();
          if (!pathInfo) {
            return -1;
          }
          return Number(pathInfo.getPathID());
        } catch (e) {
          return -1;
        }
      }
      function chooseCurrentTrafficLightCountdown(list) {
        const count = listSize(list);
        if (count <= 0) {
          return null;
        }
        const pathId = currentNaviPathId();
        if (Number.isFinite(pathId) && pathId > 0) {
          for (let i = 0; i < count; i++) {
            const item = readListItem(list, i);
            if (!item) {
              continue;
            }
            const itemPathId = Number(numberField(item, "pathID", -1));
            if (itemPathId === pathId) {
              return item;
            }
          }
        }
        return readListItem(list, 0);
      }
      function readTrafficLightState(lightInfo) {
        const states = objectField(lightInfo, "lightStates");
        const count = listSize(states);
        if (count <= 0) {
          return null;
        }
        ...
        let fallback = null;
        for (let i = 0; i < count; i++) {
          const state = readListItem(states, i);
          if (!state) {
            continue;
          }
          const rawType = Math.floor(numberField(state, "lightType", -1));
          const status = mapTrafficLightStatus(rawType);
          const stime = Math.floor(numberField(state, "stime", 0));
          const etime = Math.floor(numberField(state, "etime", 0));
          const lastSeconds = Math.floor(numberField(state, "lastSeconds", 0));
          let seconds = 0;
          if (etime > 1e9) {
            seconds = Math.max(0, etime - nowSec);
          } else if (etime > 0 && stime <= etime) {
            seconds = Math.max(0, etime);
          } else if (lastSeconds > 0) {
            seconds = lastSeconds;
          }
          const item = { rawType, status, stime, etime, lastSeconds, seconds };
          if (!fallback && status >= 0) {
            fallback = item;
          }
          if (status >= 0 && etime > 1e9 && stime <= nowSec && nowSec <= etime) {
            return item;
          }
          if (status >= 0 && etime > 1e9 && stime > nowSec && !fallback) {
            fallback = item;
          }
        }
        return fallback;
      }
      function sendTrafficLightFromCountdownList(list, source) {
        try {
          const count = listSize(list);
          if (count <= 0) {
            log(source + " traffic light empty");
            return false;
          }
          let bestPayload = null;
          const candidates = [];
          const currentItem = chooseCurrentTrafficLightCountdown(list);
          if (currentItem) {
            candidates.push(currentItem);
          }
          for (let i = 0; i < count; i++) {
            const candidate = readListItem(list, i);
          ...
          for (let i = 0; i < candidates.length; i++) {
            ...
            const lightInfo = objectField(item, "lightInfo");
            const state = readTrafficLightState(lightInfo);
            const rawDirection = Math.floor(numberField(lightInfo, "dir", -1));
            const direction = mapTrafficLightDirection(rawDirection);
            if (!state || state.status < 0 || direction < 0) {
              continue;
            }
            const seconds = Math.max(0, state.seconds);
            if (seconds <= 0 && state.status !== 4) {
              continue;
            }
            bestPayload = {
              protocolVersion: PROTOCOL_VERSION,
              ...
              status: state.status,
              seconds,
              redSeconds: state.status === 1 ? seconds : 0,
              greenSeconds: state.status === 4 ? seconds : 0,
              direction,
              rawDirection,
              waitRound: Math.max(0, Math.floor(numberField(lightInfo, "waitNum", 0))),
              distanceMeters: -1,
              rawLightType: state.rawType,
              rawLightStatus: Math.floor(numberField(item, "status", -1)),
              trafficLightId: String(numberField(item, "pathID", 0)) + ":" + String(numberField(item, "linkID", 0)) + ":" + String(numberField(item, "segmentIndex", -1)) + ":" + String(numberField(item, "linkIndex", -1)),
              lightCount: count,
              showType: Math.floor(numberField(lightInfo, "showType", -1)),
              phase: Math.floor(numberField(lightInfo, "phase", -1)),
              standardType: Math.floor(numberField(lightInfo, "standardType", -1))
            };
            break;
          }
          if (!bestPayload) {
            log(source + " traffic light invalid count=" + count + " currentPath=" + currentNaviPathId());
            return false;
          }
          const signature = [
            bestPayload.status,
            bestPayload.seconds,
            bestPayload.direction,
            bestPayload.waitRound,
            bestPayload.rawLightType,
            bestPayload.rawLightStatus,
            bestPayload.trafficLightId
          ].join(":");
          const now = Date.now();
          if (signature === keep.lastTrafficLightSignature && now - (keep.lastTrafficLightSentAt || 0) < 500) {
            return true;
          }
          keep.lastTrafficLightSignature = signature;
          keep.lastTrafficLightSentAt = now;
          sendProtocolLine("TL", bestPayload, source + " traffic-light");
          log(source + " traffic light status=" + bestPayload.status + " seconds=" + bestPayload.seconds + " dir=" + bestPayload.direction + " rawType=" + bestPayload.rawLightType + " rawStatus=" + bestPayload.rawLightStatus);
          return true;
        } catch (e) {
          log(source + " traffic light export failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function sendTrafficLightFromProtocolJson(text, source) {
        try {
          const data = jsonObjectFromString(text);
          if (!data) {
            return false;
          }
          const rawStatus = Math.floor(jsonNumber(data, "trafficLightStatus", -1));
          const status = mapProtocolTrafficLightStatus(rawStatus);
          const seconds = Math.max(0, Math.floor(jsonNumber(data, "redLightCountDownSeconds", jsonNumber(data, "greenLightLastSecond", 0))));
          const rawDirection = Math.floor(jsonNumber(data, "dir", -1));
          const direction = mapTrafficLightDirection(rawDirection);
          const waitRound = Math.max(0, Math.floor(jsonNumber(data, "waitRound", 0)));
          if (status < 0 || direction < 0 || (seconds <= 0 && status !== 2)) {
            log(source + " traffic light protocol invalid status=" + rawStatus + " seconds=" + seconds + " dir=" + rawDirection);
            return false;
          }
          const signature = [
            "protocol",
          ...
          sendProtocolLine("TL", {
            ...
            status,
            seconds,
            redSeconds: status === 1 ? seconds : 0,
            greenSeconds: status === 4 ? seconds : 0,
            direction,
            rawDirection,
            waitRound,
            distanceMeters: -1,
            rawLightType: -1,
            rawLightStatus: rawStatus,
            trafficLightId: "protocol:" + rawStatus + ":" + rawDirection,
            lightCount: 1,
            protocolStatus: rawStatus,
            trafficLightProtocol: true
          }, source + " traffic-light-protocol");
          log(source + " traffic-light-protocol status=" + status + " seconds=" + seconds + " dir=" + direction + " rawStatus=" + rawStatus);
          return true;
        } catch (e) {
          log(source + " traffic light protocol export failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function installExternalInteractionProtocolExport() {
        try {
          if (keep.externalInteractionProtocolExportInstalled) {
            return;
          }
          keep.externalInteractionProtocolExportInstalled = true;
          const Manager = frida_java_bridge_default.use("com.autosdk.common.protocol.ExternalInteractionManager");
          function handleProtocol(protocolId, data) {
            const id = Number(protocolId);
            if (id !== 80189) {
              return;
            }
            const text = data === null || data === void 0 ? "" : String(data);
            enqueueTask("xingrui:ExternalInteractionManager:" + id, function() {
              sendTrafficLightFromProtocolJson(text, "xingrui:ExternalInteractionManager:80189");
            });
          }
          const build3 = Manager.buildDataAndDispatch.overload("int", "java.lang.Object", "java.lang.String");
          build3.implementation = function(protocolId, object, data) {
            const ret = build3.call(this, protocolId, object, data);
            handleProtocol(protocolId, data);
            return ret;
          };
          const build4 = Manager.buildDataAndDispatch.overload("int", "java.lang.Object", "java.lang.String", "int");
          build4.implementation = function(protocolId, object, data, resultCode) {
            const ret = build4.call(this, protocolId, object, data, resultCode);
            handleProtocol(protocolId, data);
            return ret;
          };
          log("ExternalInteractionManager traffic light protocol export installed");
        } catch (e) {
          keep.externalInteractionProtocolExportInstalled = false;
          log("install ExternalInteractionManager protocol export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function installPresenterGuidanceExport() {
        try {
          if (keep.presenterGuidanceExportInstalled) {
            return;
          }
          keep.presenterGuidanceExportInstalled = true;
          const BasePresenter = frida_java_bridge_default.use("com.autosdk.drive.navi.presenter.BaseNaviPresenter");
          const onUpdateNaviInfo = BasePresenter.onUpdateNaviInfo.overload("java.util.ArrayList");
          onUpdateNaviInfo.implementation = function(naviInfoList) {
            const ret = onUpdateNaviInfo.call(this, naviInfoList);
            const retainedList = retainJavaObject(naviInfoList);
            const retainedPresenter = retainJavaObject(this);
            enqueueTask("xingrui:BaseNaviPresenter.onUpdateNaviInfo", function() {
              let nextRoad = "";
              try {
                nextRoad = retainedPresenter.getmDirectionInfo ? String(retainedPresenter.getmDirectionInfo()) : "";
              } catch (e) {
                nextRoad = "";
              }
              sendGuideFromNaviInfoList(retainedList, "xingrui:BaseNaviPresenter.onUpdateNaviInfo", nextRoad);
            });
            return ret;
          };
          log("BaseNaviPresenter guidance export installed");
          try {
            const DriveGuideInfoHolder = frida_java_bridge_default.use("com.autosdk.common.navi.holder.DriveGuideInfoHolder");
            const updateNextRoadDistanceView = DriveGuideInfoHolder.updateNextRoadDistanceView.overload("int", "[Ljava.lang.String;", "int");
            updateNextRoadDistanceView.implementation = function(distance, textArray, totalDistance) {
              const ret = updateNextRoadDistanceView.call(this, distance, textArray, totalDistance);
              const finalDistance = Number(distance);
              const finalTotalDistance = Number(totalDistance);
              enqueueTask("xingrui:DriveGuideInfoHolder.updateNextRoadDistanceView", function() {
                sendFinalSegmentDistance(finalDistance, finalTotalDistance, "xingrui:DriveGuideInfoHolder.updateNextRoadDistanceView");
              });
              return ret;
            };
            keep.finalGuideDistanceExportInstalled = true;
            log("DriveGuideInfoHolder final segment distance export installed");
          } catch (e) {
            keep.finalGuideDistanceExportInstalled = false;
            log("DriveGuideInfoHolder final segment distance export failed: " + (e && e.stack ? e.stack : e));
          }
        } catch (e) {
          keep.presenterGuidanceExportInstalled = false;
          log("install presenter guidance export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function installGuidanceWidgetExport() {
        try {
          if (keep.guidanceWidgetExportInstalled) {
            return;
          }
          keep.guidanceWidgetExportInstalled = true;
          const Router = frida_java_bridge_default.use("com.autonavi.gbl.guide.router.NaviObserverRouter");
          const onShowNaviLaneInfo = Router.onShowNaviLaneInfo.overload("com.autonavi.gbl.guide.model.LaneInfo");
          onShowNaviLaneInfo.implementation = function(laneInfo) {
            const ret = onShowNaviLaneInfo.call(this, laneInfo);
            const retained = retainJavaObject(laneInfo);
            enqueueTask("xingrui:NaviObserverRouter.onShowNaviLaneInfo", function() {
              sendLaneFromInfo(retained, "xingrui:NaviObserverRouter.onShowNaviLaneInfo");
            });
            return ret;
          };
          const onHideNaviLaneInfo = Router.onHideNaviLaneInfo.overload();
          onHideNaviLaneInfo.implementation = function() {
            const ret = onHideNaviLaneInfo.call(this);
            enqueueTask("xingrui:NaviObserverRouter.onHideNaviLaneInfo", function() {
              sendLaneClear("xingrui:NaviObserverRouter.onHideNaviLaneInfo");
            });
            return ret;
          };
          log("NaviObserverRouter lane export installed");
        } catch (e) {
          keep.guidanceWidgetExportInstalled = false;
          log("install NaviObserverRouter lane export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function installNaviControllerGuidanceExport() {
        try {
          if (keep.naviControllerGuidanceExportInstalled) {
            return;
          }
          keep.naviControllerGuidanceExportInstalled = true;
          const NaviController = frida_java_bridge_default.use("com.autosdk.bussiness.navi.NaviController");
          const onUpdateNaviInfo = NaviController.onUpdateNaviInfo.overload("java.util.ArrayList");
          onUpdateNaviInfo.implementation = function(naviInfoList) {
            const ret = onUpdateNaviInfo.call(this, naviInfoList);
            const retained = retainJavaObject(naviInfoList);
            enqueueTask("xingrui:NaviController.onUpdateNaviInfo", function() {
              sendGuideFromNaviInfoList(retained, "xingrui:NaviController.onUpdateNaviInfo");
            });
            return ret;
          };
          const onShowNaviLaneInfo = NaviController.onShowNaviLaneInfo.overload("com.autonavi.gbl.guide.model.LaneInfo");
          onShowNaviLaneInfo.implementation = function(laneInfo) {
            const ret = onShowNaviLaneInfo.call(this, laneInfo);
            const retained = retainJavaObject(laneInfo);
            enqueueTask("xingrui:NaviController.onShowNaviLaneInfo", function() {
              sendLaneFromInfo(retained, "xingrui:NaviController.onShowNaviLaneInfo");
            });
            return ret;
          };
          const onHideNaviLaneInfo = NaviController.onHideNaviLaneInfo.overload();
          onHideNaviLaneInfo.implementation = function() {
            const ret = onHideNaviLaneInfo.call(this);
            enqueueTask("xingrui:NaviController.onHideNaviLaneInfo", function() {
              sendLaneClear("xingrui:NaviController.onHideNaviLaneInfo");
            });
            return ret;
          };
          const onUpdateTrafficLightCountdown = NaviController.onUpdateTrafficLightCountdown.overload("java.util.ArrayList");
          onUpdateTrafficLightCountdown.implementation = function(list) {
            const ret = onUpdateTrafficLightCountdown.call(this, list);
            const retained = retainJavaObject(list);
            enqueueTask("xingrui:NaviController.onUpdateTrafficLightCountdown", function() {
              sendTrafficLightFromCountdownList(retained, "xingrui:NaviController.onUpdateTrafficLightCountdown");
            });
            return ret;
          };
          log("NaviController guidance export installed");
        } catch (e) {
          keep.naviControllerGuidanceExportInstalled = false;
          log("install NaviController guidance export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function isOfficialNavigationActive() {
        try {
          const NaviController = frida_java_bridge_default.use("com.autosdk.bussiness.navi.NaviController");
          return !!NaviController.getInstance().isNaving();
        } catch (e) {
          return false;
        }
      }
      function sendNavigationState(navigating, reason) {
        try {
          keep.officialNaviActive = !!navigating;
          let payload = null;
          if (navigating) {
            payload = restoreRouteCache();
            if (payload === null) {
              log("skip navigating state without cached route, reason=" + reason);
              return;
            }
            payload = JSON.parse(JSON.stringify(payload));
            payload.ts = Date.now();
            payload.source = "navi-state:" + reason + ":cached-route";
            payload.navigating = true;
          } else {
            keep.lastRoutePayload = null;
            keep.lastRoutePayloadSignature = "";
            keep.lastNavigatingRouteSignature = "";
            payload = {
              ts: Date.now(),
              source: "navi-state:" + reason,
              navigating: false,
              selectIndex: 0,
              routeCount: 0,
              pointCount: 0,
          ...
          const sent = writeRoutePayload(payload, "navi-state:" + reason);
          if (sent && navigating) {
            keep.lastNavigatingRouteSignature = routePayloadSignature(payload);
            keep.lastNaviRouteResendAt = Date.now();
          }
        } catch (e) {
          log("send navigation state failed: " + e);
        }
      }
      function startNavigationStatePolling() {
        if (keep.naviStatePollingStarted) {
          return;
        }
        keep.naviStatePollingStarted = true;
        keep.officialNaviActive = isOfficialNavigationActive();
        setInterval(function() {
          try {
            const active = isOfficialNavigationActive();
            if (active !== !!keep.officialNaviActive) {
              sendNavigationState(active, "poll");
            } else if (active) {
              const now = Date.now();
              const cached = restoreRouteCache();
              const signature = routePayloadSignature(cached);
              if (cached !== null && signature.length > 0 && (signature !== keep.lastNavigatingRouteSignature || now - (keep.lastNaviRouteResendAt || 0) > 2e3)) {
                const payload = JSON.parse(JSON.stringify(cached));
                payload.ts = now;
                payload.source = "navi-state:poll:active-route-refresh";
                payload.navigating = true;
                if (writeRoutePayload(payload, "navi-state:active-route-refresh")) {
                  keep.lastNavigatingRouteSignature = signature;
                  keep.lastNaviRouteResendAt = now;
                }
              }
            }
          } catch (e) {
            log("navigation state poll failed: " + e);
          }
        }, 1e3);
        log("navigation state polling started");
      }
      function installRoutePointExport() {
        try {
          let jni = function(envPtr) {
            const table = envPtr.readPointer();
            function fn(index, ret, args) {
              return new NativeFunction(table.add(index * Process.pointerSize).readPointer(), ret, args);
            }
            return {
              GetObjectClass: fn(31, "pointer", ["pointer", "pointer"]),
              GetMethodID: fn(33, "pointer", ["pointer", "pointer", "pointer", "pointer"]),
              CallObjectMethodA: fn(36, "pointer", ["pointer", "pointer", "pointer", "pointer"]),
              CallIntMethodA: fn(51, "int", ["pointer", "pointer", "pointer", "pointer"]),
              CallLongMethodA: fn(54, "int64", ["pointer", "pointer", "pointer", "pointer"]),
              GetFieldID: fn(94, "pointer", ["pointer", "pointer", "pointer", "pointer"]),
              GetObjectField: fn(95, "pointer", ["pointer", "pointer", "pointer"]),
              GetIntField: fn(100, "int", ["pointer", "pointer", "pointer"])
            };
          }, cstr = function(value) {
            return Memory.allocUtf8String(value);
          }, jargs = function(values) {
            const p = Memory.alloc(Process.pointerSize * values.length);
            for (let i = 0; i < values.length; i++) {
              p.add(i * Process.pointerSize).writeS64(values[i]);
            }
            return p;
          };
          if (keep.routePointExportInstalled) {
            return;
          }
          const gbl = Process.findModuleByName("libGbl.so");
          if (gbl === null) {
            log("route export skipped: libGbl.so not loaded");
            return;
          }
          keep.routePointExportInstalled = true;
          const setPathInfos = gbl.base.add(ROUTE_EXPORT_OFFSET);
          log("route export hook setPathInfosNative @" + setPathInfos);
          Interceptor.attach(setPathInfos, {
            onEnter(args) {
              try {
                const now = Date.now();
                const active = isOfficialNavigationActive();
                if (now - (keep.lastNativeRouteEnterLogAt || 0) > 1e3) {
                  keep.lastNativeRouteEnterLogAt = now;
                  log("setPathInfosNative enter select=" + args[5].toInt32() + ", naving=" + active);
                }
                if (keep.routePointExportInProgress) {
                  return;
                }
                keep.routePointExportInProgress = true;
                const envPtr = args[0];
                const list = args[4];
                const select = args[5].toInt32();
                if (list.isNull()) {
                  return;
                }
                const e = jni(envPtr);
                const listCls = e.GetObjectClass(envPtr, list);
                const sizeMid = e.GetMethodID(envPtr, listCls, cstr("size"), cstr("()I"));
                const getMid = e.GetMethodID(envPtr, listCls, cstr("get"), cstr("(I)Ljava/lang/Object;"));
                const count = e.CallIntMethodA(envPtr, list, sizeMid, ptr(0));
                log("setPathInfosNative list count=" + count + ", select=" + select);
                if (count <= 0) {
                  return;
                }
                const routes = [];
                const selectedIndex = select >= 0 && select < count ? select : 0;
                for (let i = 0; i < count; i++) {
                  let appendPointList = function(pointsList) {
                    if (pointsList.isNull()) {
                      return 0;
                    }
                    const plCls = e.GetObjectClass(envPtr, pointsList);
                    const plSize = e.GetMethodID(envPtr, plCls, cstr("size"), cstr("()I"));
                    const plGet = e.GetMethodID(envPtr, plCls, cstr("get"), cstr("(I)Ljava/lang/Object;"));
                    const pointCount2 = e.CallIntMethodA(envPtr, pointsList, plSize, ptr(0));
                    let appended = 0;
                    for (let p = 0; p < pointCount2; p++) {
                      const cp = e.CallObjectMethodA(envPtr, pointsList, plGet, jargs([p]));
                      if (cp.isNull()) {
                        continue;
                      }
                      const cpCls = e.GetObjectClass(envPtr, cp);
                      const lonF = e.GetFieldID(envPtr, cpCls, cstr("lon"), cstr("I"));
                      const latF = e.GetFieldID(envPtr, cpCls, cstr("lat"), cstr("I"));
                      route.points.push([e.GetIntField(envPtr, cp, lonF), e.GetIntField(envPtr, cp, latF)]);
                      appended++;
                    }
                    return appended;
                  };
                  const attrs = e.CallObjectMethodA(envPtr, list, getMid, jargs([i]));
                  if (attrs.isNull()) {
                    continue;
                  }
                  const attrsCls = e.GetObjectClass(envPtr, attrs);
                  const pathField = e.GetFieldID(envPtr, attrsCls, cstr("mPathInfo"), cstr("Lcom/autonavi/gbl/common/path/option/PathInfo;"));
                  const path = e.GetObjectField(envPtr, attrs, pathField);
                  if (path.isNull()) {
                    continue;
                  }
                  const pathCls = e.GetObjectClass(envPtr, path);
                  const lengthMid = e.GetMethodID(envPtr, pathCls, cstr("getLength"), cstr("()J"));
                  const timeMid = e.GetMethodID(envPtr, pathCls, cstr("getTravelTime"), cstr("()J"));
                  const lightMid = e.GetMethodID(envPtr, pathCls, cstr("getTrafficLightCount"), cstr("()J"));
                  const tollMid = e.GetMethodID(envPtr, pathCls, cstr("getTollCost"), cstr("()I"));
                  const pathIdMid = e.GetMethodID(envPtr, pathCls, cstr("getPathID"), cstr("()J"));
                  const segCountMid = e.GetMethodID(envPtr, pathCls, cstr("getSegmentCount"), cstr("()J"));
                  const getSegMid = e.GetMethodID(envPtr, pathCls, cstr("getSegmentInfo"), cstr("(J)Lcom/autonavi/gbl/common/path/option/SegmentInfo;"));
                  const route = {
                    index: i,
                    selected: i === selectedIndex,
                    pathId: Number(e.CallLongMethodA(envPtr, path, pathIdMid, ptr(0))),
                    length: Number(e.CallLongMethodA(envPtr, path, lengthMid, ptr(0))),
                    travelTime: Number(e.CallLongMethodA(envPtr, path, timeMid, ptr(0))),
                    trafficLightCount: Number(e.CallLongMethodA(envPtr, path, lightMid, ptr(0))),
                    tollCost: Number(e.CallIntMethodA(envPtr, path, tollMid, ptr(0))),
                    points: []
                  };
                  const segCount = Number(e.CallLongMethodA(envPtr, path, segCountMid, ptr(0)));
                  route.signature = [
                    route.index,
                    route.pathId,
                    route.length,
                    route.travelTime,
                    route.trafficLightCount,
                    route.tollCost,
                    segCount
                  ].join(":");
                  for (let s = 0; s < segCount; s++) {
                    const seg = e.CallObjectMethodA(envPtr, path, getSegMid, jargs([s]));
                    if (seg.isNull()) {
                      continue;
                    }
                    const segCls = e.GetObjectClass(envPtr, seg);
                    const linkCountMid = e.GetMethodID(envPtr, segCls, cstr("getLinkCount"), cstr("()J"));
                    const getLinkMid = e.GetMethodID(envPtr, segCls, cstr("getLinkInfo"), cstr("(J)Lcom/autonavi/gbl/common/path/option/LinkInfo;"));
                    const segPointsMid = e.GetMethodID(envPtr, segCls, cstr("getPoints"), cstr("()Ljava/util/ArrayList;"));
                    let appendedInSegment = 0;
                    const linkCount = Number(e.CallLongMethodA(envPtr, seg, linkCountMid, ptr(0)));
                    for (let l = 0; l < linkCount; l++) {
                      const link = e.CallObjectMethodA(envPtr, seg, getLinkMid, jargs([l]));
                      if (link.isNull()) {
                        continue;
                      }
                      const linkCls = e.GetObjectClass(envPtr, link);
                      const linkPointsMid = e.GetMethodID(envPtr, linkCls, cstr("getPoints"), cstr("()Ljava/util/ArrayList;"));
                      const linkPoints = e.CallObjectMethodA(envPtr, link, linkPointsMid, ptr(0));
                      appendedInSegment += appendPointList(linkPoints);
                    }
                    if (appendedInSegment === 0) {
                      appendPointList(e.CallObjectMethodA(envPtr, seg, segPointsMid, ptr(0)));
                    }
                  }
                  routes.push(route);
                }
                const pointCount = routes.reduce(function(total, route) {
                  return total + (route.points ? route.points.length : 0);
                }, 0);
                if (pointCount > 1) {
                  const payload = {
                    ts: Date.now(),
                    source: "native:setPathInfosNative:selected-live",
                    navigating: isOfficialNavigationActive(),
                    selectIndex: selectedIndex,
                    routeCount: count,
                    pointCount,
                    routes
                  };
                  keep.nativeRoutePayload = JSON.parse(JSON.stringify(payload));
                  log("native route cache updated routes=" + routes.length + ", selected=" + selectedIndex + ", points=" + pointCount);
                  writeRoutePayload(payload, "route export");
                }
              } catch (e) {
                log("route export failed: " + e.stack);
              } finally {
                keep.routePointExportInProgress = false;
              }
            }
          });
        } catch (e) {
          keep.routePointExportInstalled = false;
          log("install route export failed: " + e.stack);
        }
      }
      function installRoutePointExportWhenReady(attempt) {
        installRoutePointExport();
        if (!keep.routePointExportInstalled && attempt < 30) {
          setTimeout(function() {
            installRoutePointExportWhenReady(attempt + 1);
          }, 1e3);
        }
      }
      function javaNumber(value, fallback) {
        try {
      ...
      function javaListSize(list) {
        try {
          return list === null ? 0 : Number(list.size());
        } catch (e) {
          return 0;
        }
      }
      function callLongArgMethod(obj, methodName, value) {
        try {
          return obj[methodName].overload("long").call(obj, Math.max(0, Math.floor(Number(value) || 0)));
        } catch (e) {
          try {
            return obj[methodName](Math.max(0, Math.floor(Number(value) || 0)));
          } catch (ignored) {
            return null;
          }
        }
      }
      function javaFieldValue(obj, fieldName, fallback) {
        try {
      ...
      function getPathInfoFromAttrs(attrs) {
        if (attrs === null || attrs === void 0) {
          return null;
        }
        try {
          if (attrs.mPathInfo !== void 0) {
            return attrs.mPathInfo.value;
          }
        } catch (e) {
        }
        try {
          return attrs.getPathInfo();
        } catch (e) {
          return null;
        }
      }
      function appendJavaPointList(route, pointsList) {
        const count = javaListSize(pointsList);
        let appended = 0;
        for (let i = 0; i < count; i++) {
          try {
            const point = pointsList.get(i);
            if (point === null) {
              continue;
            }
            route.points.push([
              javaNumber(point.lon.value, 0),
              javaNumber(point.lat.value, 0)
            ]);
            appended++;
          } catch (e) {
          }
      ...
      function buildRouteFromJavaPath(path, index, selected) {
        let pathId = 0;
        let pathIndex = index;
        let naviId = "";
        try {
          pathId = javaNumber(path.getPathID(), 0);
        } catch (e) {
          pathId = 0;
        }
        try {
          pathIndex = javaNumber(path.getPathIndex(), index);
        } catch (e) {
          pathIndex = index;
        }
        try {
          naviId = String(path.getNaviID() || "");
        } catch (e) {
          naviId = "";
        }
        const route = {
          ...
          pathIndex,
          naviId,
          length: javaNumber(path.getLength(), 0),
          travelTime: javaNumber(path.getTravelTime(), 0),
          trafficLightCount: javaNumber(path.getTrafficLightCount(), 0),
          tollCost: javaNumber(path.getTollCost(), 0),
          points: []
        };
        const segCount = javaNumber(path.getSegmentCount(), 0);
        route.segmentCount = segCount;
        route.debugSegmentHitCount = 0;
        route.debugLinkHitCount = 0;
        route.debugSegmentPointCount = 0;
        route.debugLinkPointCount = 0;
        route.signature = [
          route.index,
          route.pathId,
          route.pathIndex,
        ...
        for (let s = 0; s < segCount; s++) {
          try {
            const seg = callLongArgMethod(path, "getSegmentInfo", s);
            if (seg === null) {
              continue;
            }
            route.debugSegmentHitCount++;
            let appendedInSegment = 0;
            const linkCount = javaNumber(seg.getLinkCount(), 0);
            for (let l = 0; l < linkCount; l++) {
              try {
                const link = callLongArgMethod(seg, "getLinkInfo", l);
                if (link !== null) {
                  route.debugLinkHitCount++;
                  const appendedInLink = appendJavaPointList(route, link.getPoints());
                  route.debugLinkPointCount += appendedInLink;
                  appendedInSegment += appendedInLink;
                }
              } catch (e) {
              }
            }
            if (appendedInSegment === 0) {
              const appendedFromSegment = appendJavaPointList(route, seg.getPoints());
              route.debugSegmentPointCount += appendedFromSegment;
            }
          } catch (e) {
          }
        }
      ...
      function exportRouteFromJavaPathInfos(pathInfos, selectIndex, source) {
        try {
          const count = javaListSize(pathInfos);
          if (count <= 0) {
            return;
          }
          const selectedIndex = selectIndex >= 0 && selectIndex < count ? selectIndex : 0;
          const attrs = pathInfos.get(selectedIndex);
          const path = getPathInfoFromAttrs(attrs);
          if (path === null) {
            log(source + " skip: selected path null");
            return;
          }
          const route = buildRouteFromJavaPath(path, selectedIndex, selectedIndex === selectIndex);
          const pointCount = route.points.length;
          if (pointCount <= 1) {
            log(source + " skip: no route points");
            return;
          }
          writeRoutePayload({
            ts: Date.now(),
          ...
          log(source + " export failed: " + e.stack);
        }
      }
      function exportRouteFromJavaPathList(pathList, selectIndex, source, navigatingOverride) {
        try {
          const count = javaListSize(pathList);
          if (count <= 0) {
            log(source + " skip: path list empty");
            return false;
          }
          const selectedIndex = selectIndex >= 0 && selectIndex < count ? selectIndex : 0;
          const path = pathList.get(selectedIndex);
          if (path === null) {
            log(source + " skip: selected path null");
            return false;
          }
          const route = buildRouteFromJavaPath(path, selectedIndex, true);
          const pointCount = route.points.length;
          androidLog(source + " pathList route points=" + pointCount
            + " selected=" + selectedIndex
            + " routeCount=" + count
            + " segments=" + route.segmentCount
            + " segmentHits=" + route.debugSegmentHitCount
            + " linkHits=" + route.debugLinkHitCount
            + " linkPoints=" + route.debugLinkPointCount
            + " segmentPoints=" + route.debugSegmentPointCount
            + " length=" + route.length
          ...
          return writeRoutePayload({
            ts: Date.now(),
            source,
            navigating: navigatingOverride === void 0 ? isOfficialNavigationActive() : !!navigatingOverride,
            selectIndex: selectedIndex,
            routeCount: count,
            pointCount,
            routes: [route]
          ...
          log(source + " export path list failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function exportRouteFromNaviPath(naviPath, source, navigatingOverride) {
        try {
          if (naviPath === null || naviPath === void 0) {
            log(source + " skip: naviPath null");
            return false;
          }
          const vecPaths = javaFieldValue(naviPath, "vecPaths", null);
          const mainIdx = Number(javaFieldValue(naviPath, "mainIdx", 0));
          return exportRouteFromJavaPathList(vecPaths, isNaN(mainIdx) ? 0 : mainIdx, source, navigatingOverride);
        } catch (e) {
          log(source + " export naviPath failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function exportRouteFromNaviPathByPathId(naviPath, pathId, source, navigatingOverride) {
        try {
          if (naviPath === null || naviPath === void 0) {
            log(source + " skip: naviPath null");
            return false;
          ...
          const vecPaths = javaFieldValue(naviPath, "vecPaths", null);
          const count = javaListSize(vecPaths);
          if (count <= 0) {
            log(source + " skip: path list empty");
            return false;
          }
          ...
          for (let i = 0; i < count; i++) {
            try {
              const path = vecPaths.get(i);
              if (path !== null && String(path.getPathID()) === target) {
                selectedIndex = i;
                break;
              }
            } catch (ignored) {
          ...
          if (selectedIndex < 0) {
            const mainIdx = Number(javaFieldValue(naviPath, "mainIdx", 0));
            selectedIndex = isNaN(mainIdx) ? 0 : mainIdx;
            log(source + " pathId=" + target + " not found, fallback index=" + selectedIndex);
          } else {
            log(source + " matched pathId=" + target + " index=" + selectedIndex);
          }
          return exportRouteFromJavaPathList(vecPaths, selectedIndex, source, navigatingOverride);
        } catch (e) {
          log(source + " export by pathId failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function currentRouteCarResultData() {
        try {
          const ModuleServiceUtils = frida_java_bridge_default.use("com.autosdk.common.utils.ModuleServiceUtils");
          const driveService = ModuleServiceUtils.getDriveService();
          if (driveService) {
            const result = driveService.getRouteCarResultData();
            if (result) {
              return result;
            }
          }
        ...
        try {
          const RouteRequestController = frida_java_bridge_default.use("com.autosdk.bussiness.navi.route.RouteRequestController");
          const controller = RouteRequestController.getInstance();
          if (controller) {
            const result = controller.getmCarRouteResult();
            if (result) {
              return result;
            }
          }
      ...
      function exportRouteFromRouteCarResultData(result, source, navigatingOverride) {
        try {
          if (result === null || result === void 0) {
            log(source + " skip: route result null");
            return false;
          }
          const paths = result.getPathResult();
          let focusIndex = 0;
          try {
            focusIndex = Number(result.getFocusIndex());
          } catch (e) {
            focusIndex = 0;
          }
          return exportRouteFromJavaPathList(paths, isNaN(focusIndex) ? 0 : focusIndex, source, navigatingOverride);
        } catch (e) {
          log(source + " export route result failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function exportRouteFromNaviComponentDrawRoute(routeData, pathInfos, source, navigatingOverride) {
        try {
          let focusIndex = 0;
          try {
            focusIndex = Number(routeData.getFocusIndex());
          } catch (e) {
            focusIndex = 0;
          }
          if (javaListSize(pathInfos) > 0) {
            return exportRouteFromJavaPathList(pathInfos, isNaN(focusIndex) ? 0 : focusIndex, source, navigatingOverride);
          }
          return exportRouteFromRouteCarResultData(routeData, source + ":routeDataFallback", navigatingOverride);
        } catch (e) {
          log(source + " export NaviComponent drawRoute failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function exportPathInfo(pathInfo, source, navigatingOverride) {
        try {
          if (pathInfo === null || pathInfo === void 0) {
            log(source + " skip: pathInfo null");
            return false;
          }
          const route = buildRouteFromJavaPath(pathInfo, 0, true);
          const pointCount = route.points.length;
          androidLog(source + " pathInfo route points=" + pointCount
            + " segments=" + route.segmentCount
            + " segmentHits=" + route.debugSegmentHitCount
            + " linkHits=" + route.debugLinkHitCount
            + " linkPoints=" + route.debugLinkPointCount
            ...
            + " time=" + route.travelTime);
          if (pointCount <= 1) {
            log(source + " skip: pathInfo no route points");
            return false;
          }
          return writeRoutePayload({
            ts: Date.now(),
            source,
            navigating: navigatingOverride === void 0 ? true : !!navigatingOverride,
            selectIndex: 0,
            routeCount: 1,
            pointCount,
            routes: [route]
          ...
          log(source + " export pathInfo failed: " + (e && e.stack ? e.stack : e));
          return false;
        }
      }
      function currentDriveService() {
        try {
          const ModuleServiceUtils = frida_java_bridge_default.use("com.autosdk.common.utils.ModuleServiceUtils");
          return ModuleServiceUtils.getDriveService();
        } catch (e) {
          return null;
        }
      }
      function exportCurrentNavigationRoute(source, navigatingOverride) {
        try {
          const navigating = navigatingOverride === void 0 ? true : !!navigatingOverride;
          androidLog(source + " route snapshot begin navigating=" + navigating);
          const result = currentRouteCarResultData();
          if (result !== null) {
            androidLog(source + " route snapshot using RouteCarResultData");
            return exportRouteFromRouteCarResultData(result, source + ":currentResult", navigating);
          }
          androidLog(source + " route snapshot RouteCarResultData null");
          const driveService = currentDriveService();
          if (driveService) {
            try {
              const pathInfo = driveService.getNaviPathInfo();
              if (pathInfo) {
                androidLog(source + " route snapshot using driveService.getNaviPathInfo");
                return exportPathInfo(pathInfo, source + ":driveServiceNaviPathInfo", navigating);
              }
              androidLog(source + " route snapshot driveService.getNaviPathInfo null");
            } catch (e) {
              log(source + " driveService getNaviPathInfo failed: " + (e && e.stack ? e.stack : e));
              androidLog(source + " route snapshot driveService.getNaviPathInfo failed");
            }
          } else {
            androidLog(source + " route snapshot driveService null");
          }
          const NaviController = frida_java_bridge_default.use("com.autosdk.bussiness.navi.NaviController");
          const naviPath = NaviController.getInstance().getNaviPath();
          androidLog(source + " route snapshot using NaviController.getNaviPath");
          return exportRouteFromNaviPath(naviPath, source + ":currentNaviPath", navigating);
        } catch (e) {
          log(source + " export current navigation route failed: " + (e && e.stack ? e.stack : e));
          androidLog(source + " route snapshot failed");
          return false;
        }
      }
      function scheduleCurrentNavigationRouteExport(source) {
        [0, 120, 450, 900].forEach(function(delay) {
          const label = source + ":currentRoute:" + delay;
          if (delay <= 0) {
            enqueueTask(label, function() {
              exportCurrentNavigationRoute(source);
            });
          } else {
            setTimeout(function() {
              enqueueTask(label, function() {
                exportCurrentNavigationRoute(source + ":delayed:" + delay);
              });
            }, delay);
          }
        });
      }
      function scheduleGuideCurrentRouteExport(source) {
        const now = Date.now();
        if (now - (keep.lastGuideCurrentRouteExportAt || 0) < 2e3) {
          return;
        }
        keep.lastGuideCurrentRouteExportAt = now;
        androidLog(source + " route snapshot scheduled");
        scheduleCurrentNavigationRouteExport(source);
      }
      function scheduleCurrentNaviPathExport(source) {
        scheduleCurrentNavigationRouteExport(source);
      }
      function installDrawRouteSelectionExport() {
        try {
          if (keep.drawRouteSelectionExportInstalled) {
            return;
          }
          keep.drawRouteSelectionExportInstalled = true;
          const DrivingLayer = frida_java_bridge_default.use("com.autosdk.bussiness.layer.DrivingLayer");
          const drawRoute5 = DrivingLayer.drawRoute.overload(
            "com.autonavi.gbl.common.path.model.RoutePoints",
            "java.util.ArrayList",
            "int",
            "boolean",
            "long"
          );
          drawRoute5.implementation = function(routePoints, pathInfos, selectIndex, isPreview, totalDistance) {
            const ret = drawRoute5.call(this, routePoints, pathInfos, selectIndex, isPreview, totalDistance);
            try {
              const surfaceId = this.getmSurfaceViewID ? this.getmSurfaceViewID() : -999;
              const retainedPathInfos = retainJavaObject(pathInfos);
              const selected = Number(selectIndex);
              enqueueTask("xingrui:DrivingLayer.drawRoute5", function() {
                const source = "xingrui:DrivingLayer.drawRoute5:s" + surfaceId;
                exportRouteFromJavaPathList(retainedPathInfos, isNaN(selected) ? 0 : selected, source, true);
              });
              log("drawRoute5 surfaceId=" + surfaceId + ", select=" + selectIndex + ", preview=" + isPreview + ", pathCount=" + javaListSize(pathInfos));
            } catch (e) {
              log("drawRoute5 selection export failed: " + e);
            }
            return ret;
          };
          const drawRoute6 = DrivingLayer.drawRoute.overload(
            "com.autonavi.gbl.common.path.model.RoutePoints",
            "java.util.ArrayList",
            "int",
            "boolean",
          ...
          drawRoute6.implementation = function(routePoints, pathInfos, selectIndex, isPreview, totalDistance, showArrow) {
            const ret = drawRoute6.call(this, routePoints, pathInfos, selectIndex, isPreview, totalDistance, showArrow);
            try {
              const surfaceId = this.getmSurfaceViewID ? this.getmSurfaceViewID() : -999;
              const retainedPathInfos = retainJavaObject(pathInfos);
              const selected = Number(selectIndex);
              enqueueTask("xingrui:DrivingLayer.drawRoute6", function() {
                const source = "xingrui:DrivingLayer.drawRoute6:s" + surfaceId;
                exportRouteFromJavaPathList(retainedPathInfos, isNaN(selected) ? 0 : selected, source, true);
              });
              log("drawRoute6 surfaceId=" + surfaceId + ", select=" + selectIndex + ", preview=" + isPreview + ", pathCount=" + javaListSize(pathInfos) + ", showArrow=" + showArrow);
            } catch (e) {
              log("drawRoute6 selection export failed: " + e);
            }
            return ret;
          };
          log("DrivingLayer drawRoute selection export installed");
        } catch (e) {
          keep.drawRouteSelectionExportInstalled = false;
          log("install drawRoute selection export failed: " + e.stack);
        }
      }
      function installNaviControllerRouteExport() {
        try {
          if (keep.naviControllerRouteExportInstalled) {
            return;
          }
          keep.naviControllerRouteExportInstalled = true;
          const NaviController = frida_java_bridge_default.use("com.autosdk.bussiness.navi.NaviController");
          const onMainNaviPath = NaviController.onMainNaviPath.overload(
            "com.autonavi.gbl.common.path.option.PathInfo"
          );
          onMainNaviPath.implementation = function(path) {
            const ret = onMainNaviPath.call(this, path);
            try {
              if (path !== null) {
                const route = buildRouteFromJavaPath(path, 0, true);
                const pointCount = route.points.length;
                log("onMainNaviPath pointCount=" + pointCount + ", naving=" + isOfficialNavigationActive());
                if (pointCount > 1) {
                  writeRoutePayload({
                    ts: Date.now(),
                    source: "java:NaviController.onMainNaviPath",
                    navigating: true,
                    selectIndex: 0,
                    routeCount: 1,
                    pointCount,
                    routes: [route]
                  }, "java:NaviController.onMainNaviPath");
                }
              }
            } catch (e) {
              log("onMainNaviPath export failed: " + e.stack);
            }
            return ret;
          };
          const setNaviPath = NaviController.setNaviPath.overload(
            "com.autonavi.gbl.guide.model.NaviPath"
          );
          setNaviPath.implementation = function(naviPath) {
            const ret = setNaviPath.call(this, naviPath);
            try {
              const retained = retainJavaObject(naviPath);
              enqueueTask("xingrui:NaviController.setNaviPath", function() {
                exportRouteFromNaviPath(retained, "java:NaviController.setNaviPath", isOfficialNavigationActive());
              });
              scheduleCurrentNaviPathExport("java:NaviController.setNaviPath");
            } catch (e) {
              log("setNaviPath route export failed: " + (e && e.stack ? e.stack : e));
            }
            return ret;
          };
          const startNaviByPathId = NaviController.startNavi.overload("long", "int");
          startNaviByPathId.implementation = function(naviId, type) {
            const ret = startNaviByPathId.call(this, naviId, type);
            try {
              const selectedPathId = String(naviId);
              enqueueTask("xingrui:NaviController.startNavi:pathId", function() {
                try {
                  const naviPath = NaviController.getInstance().getNaviPath();
                  exportRouteFromNaviPathByPathId(naviPath, selectedPathId, "java:NaviController.startNavi:pathId", true);
                } catch (e) {
                  log("startNavi pathId export task failed: " + (e && e.stack ? e.stack : e));
                }
              });
              [180, 600, 1200].forEach(function(delay) {
                setTimeout(function() {
                  enqueueTask("xingrui:NaviController.startNavi:pathId:delayed:" + delay, function() {
                    try {
                      const naviPath = NaviController.getInstance().getNaviPath();
                      exportRouteFromNaviPathByPathId(naviPath, selectedPathId, "java:NaviController.startNavi:pathId:delayed:" + delay, true);
                    } catch (e) {
                      log("startNavi pathId delayed export failed: " + (e && e.stack ? e.stack : e));
                    }
                  });
                }, delay);
              });
            } catch (e) {
              log("startNavi pathId route export failed: " + (e && e.stack ? e.stack : e));
            }
            return ret;
          };
          log("NaviController onMainNaviPath export installed");
        } catch (e) {
          keep.naviControllerRouteExportInstalled = false;
          log("install NaviController route export failed: " + e.stack);
        }
      }
      function installNaviComponentRouteExport() {
        try {
          if (keep.naviComponentRouteExportInstalled) {
            return;
          }
          keep.naviComponentRouteExportInstalled = true;
          const NaviComponent = frida_java_bridge_default.use("com.autosdk.bussiness.widget.navi.NaviComponent");
          const drawRoute3 = NaviComponent.drawRoute.overload(
            "com.autosdk.bussiness.navi.route.model.RouteCarResultData",
            "java.util.ArrayList",
            "int"
          );
          drawRoute3.implementation = function(routeData, pathInfos, naviType) {
            const ret = drawRoute3.call(this, routeData, pathInfos, naviType);
            try {
              const retainedRouteData = retainJavaObject(routeData);
              const retainedPathInfos = retainJavaObject(pathInfos);
              enqueueTask("xingrui:NaviComponent.drawRoute3", function() {
                exportRouteFromNaviComponentDrawRoute(retainedRouteData, retainedPathInfos, "java:NaviComponent.drawRoute3", isOfficialNavigationActive());
              });
            } catch (e) {
              log("NaviComponent.drawRoute3 export failed: " + (e && e.stack ? e.stack : e));
            }
            return ret;
          };
          const drawRoute4 = NaviComponent.drawRoute.overload(
            "com.autosdk.bussiness.navi.route.model.RouteCarResultData",
            "java.util.ArrayList",
            "int",
            "int"
          );
          drawRoute4.implementation = function(routeData, pathInfos, naviType, surfaceViewID) {
            const ret = drawRoute4.call(this, routeData, pathInfos, naviType, surfaceViewID);
            try {
              const retainedRouteData = retainJavaObject(routeData);
              const retainedPathInfos = retainJavaObject(pathInfos);
              enqueueTask("xingrui:NaviComponent.drawRoute4", function() {
                exportRouteFromNaviComponentDrawRoute(retainedRouteData, retainedPathInfos, "java:NaviComponent.drawRoute4:s" + surfaceViewID, isOfficialNavigationActive());
              });
            } catch (e) {
              log("NaviComponent.drawRoute4 export failed: " + (e && e.stack ? e.stack : e));
            }
            return ret;
          };
          log("NaviComponent drawRoute route export installed");
        } catch (e) {
          keep.naviComponentRouteExportInstalled = false;
          log("install NaviComponent route export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function installNaviComponentPathExport() {
        try {
          if (keep.naviComponentPathExportInstalled) {
            return;
          }
          keep.naviComponentPathExportInstalled = true;
          const NaviComponent = frida_java_bridge_default.use("com.autosdk.bussiness.widget.navi.NaviComponent");
          const setNaviPath = NaviComponent.setNaviPath.overload(
            "com.autosdk.bussiness.navi.route.model.RouteCarResultData",
            "java.util.ArrayList",
            "int"
          );
          setNaviPath.implementation = function(routeData, pathInfos, naviType) {
            const ret = setNaviPath.call(this, routeData, pathInfos, naviType);
            try {
              const retainedRouteData = retainJavaObject(routeData);
              const retainedPathInfos = retainJavaObject(pathInfos);
              enqueueTask("xingrui:NaviComponent.setNaviPath", function() {
                const source = "java:NaviComponent.setNaviPath";
                if (!exportRouteFromNaviComponentDrawRoute(retainedRouteData, retainedPathInfos, source, isOfficialNavigationActive())) {
                  exportRouteFromRouteCarResultData(retainedRouteData, source + ":routeDataFallback", isOfficialNavigationActive());
                }
              });
            } catch (e) {
              log("NaviComponent.setNaviPath export failed: " + (e && e.stack ? e.stack : e));
            }
            return ret;
          };
          const setRoute = NaviComponent.setRoute.overload(
            "com.autosdk.bussiness.navi.route.model.RouteCarResultData",
            "java.util.ArrayList",
            "int",
            "int"
          );
          setRoute.implementation = function(routeData, pathInfos, naviType, routeType) {
            const ret = setRoute.call(this, routeData, pathInfos, naviType, routeType);
            try {
              const retainedRouteData = retainJavaObject(routeData);
              const retainedPathInfos = retainJavaObject(pathInfos);
              enqueueTask("xingrui:NaviComponent.setRoute", function() {
                const source = "java:NaviComponent.setRoute";
                if (!exportRouteFromNaviComponentDrawRoute(retainedRouteData, retainedPathInfos, source, isOfficialNavigationActive())) {
                  exportRouteFromRouteCarResultData(retainedRouteData, source + ":routeDataFallback", isOfficialNavigationActive());
                }
              });
            } catch (e) {
              log("NaviComponent.setRoute export failed: " + (e && e.stack ? e.stack : e));
            }
            return ret;
          };
          log("NaviComponent setNaviPath/setRoute export installed");
        } catch (e) {
          keep.naviComponentPathExportInstalled = false;
          log("install NaviComponent path export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function installGeelyRouteLayerExport() {
        try {
          if (keep.geelyRouteLayerExportInstalled) {
            return;
          }
          keep.geelyRouteLayerExportInstalled = true;
          function hookRouteLayer(className) {
            try {
              const Manager = frida_java_bridge_default.use(className);
              try {
                const drawRouteCar = Manager.drawRoute.overload(
                  "com.autosdk.bussiness.navi.route.model.RouteCarResultData",
                  "java.util.ArrayList",
                  "int"
                );
                drawRouteCar.implementation = function(routeData, pathInfos, carType) {
                  const ret = drawRouteCar.call(this, routeData, pathInfos, carType);
                  try {
                    const retainedRouteData = retainJavaObject(routeData);
                    const retainedPathInfos = retainJavaObject(pathInfos);
                    enqueueTask("xingrui:geely-route-layer:" + className + ".drawRouteCar", function() {
                      const source = "geely:route-layer:" + className.substring(className.lastIndexOf(".") + 1) + ".drawRouteCar";
                      if (!exportRouteFromNaviComponentDrawRoute(retainedRouteData, retainedPathInfos, source, isOfficialNavigationActive())) {
                        exportRouteFromRouteCarResultData(retainedRouteData, source + ":routeDataFallback", isOfficialNavigationActive());
                      }
                    });
                  } catch (e) {
                    log(className + ".drawRouteCar export failed: " + (e && e.stack ? e.stack : e));
                  }
                  return ret;
                };
                log(className + " drawRouteCar export installed");
              } catch (e) {
                log(className + " drawRouteCar hook unavailable: " + (e && e.stack ? e.stack : e));
              }
              try {
                const drawRoutePoints = Manager.drawRoute.overload(
                  "com.autonavi.gbl.common.path.model.RoutePoints",
                  "java.util.ArrayList",
                  "int",
                  "boolean",
                ...
                drawRoutePoints.implementation = function(pathPoints, pathInfos, selectIndex, isShowAll, totalDistance) {
                  const ret = drawRoutePoints.call(this, pathPoints, pathInfos, selectIndex, isShowAll, totalDistance);
                  try {
                    const retainedPathInfos = retainJavaObject(pathInfos);
                    const selected = Number(selectIndex);
                    enqueueTask("xingrui:geely-route-layer:" + className + ".drawRoutePoints", function() {
                      const source = "geely:route-layer:" + className.substring(className.lastIndexOf(".") + 1) + ".drawRoutePoints";
                      exportRouteFromJavaPathList(retainedPathInfos, isNaN(selected) ? 0 : selected, source, isOfficialNavigationActive());
                    });
                  } catch (e) {
                    log(className + ".drawRoutePoints export failed: " + (e && e.stack ? e.stack : e));
                  }
                  return ret;
                };
                log(className + " drawRoutePoints export installed");
              } catch (e) {
                log(className + " drawRoutePoints hook unavailable: " + (e && e.stack ? e.stack : e));
              }
              try {
                const drawRouteResult = Manager.drawRouteResult.overload(
                  "com.autosdk.bussiness.navi.route.model.RouteCarResultData",
                  "java.util.ArrayList"
                );
                drawRouteResult.implementation = function(routeData, pathInfos) {
                  const ret = drawRouteResult.call(this, routeData, pathInfos);
                  try {
                    const retainedRouteData = retainJavaObject(routeData);
                    const retainedPathInfos = retainJavaObject(pathInfos);
                    enqueueTask("xingrui:geely-route-layer:" + className + ".drawRouteResult", function() {
                      const source = "geely:route-layer:" + className.substring(className.lastIndexOf(".") + 1) + ".drawRouteResult";
                      if (!exportRouteFromNaviComponentDrawRoute(retainedRouteData, retainedPathInfos, source, isOfficialNavigationActive())) {
                        exportRouteFromRouteCarResultData(retainedRouteData, source + ":routeDataFallback", isOfficialNavigationActive());
                      }
                    });
                  } catch (e) {
                    log(className + ".drawRouteResult export failed: " + (e && e.stack ? e.stack : e));
                  }
                  return ret;
                };
                log(className + " drawRouteResult export installed");
              } catch (e) {
                log(className + " drawRouteResult hook unavailable: " + (e && e.stack ? e.stack : e));
              }
            } catch (e) {
              log(className + " route layer hook failed: " + (e && e.stack ? e.stack : e));
            }
          }
          [
            "com.geely.extscreen.ExtraScreenManager",
            "com.geely.widget.WidgetExtScreenManager",
            "com.geely.hud.HudExtScreenManager"
          ].forEach(hookRouteLayer);
          log("Geely route layer export installed");
        } catch (e) {
          keep.geelyRouteLayerExportInstalled = false;
          log("install Geely route layer export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function installRouteFocusExport() {
        try {
          if (keep.routeFocusExportInstalled) {
            return;
          }
          keep.routeFocusExportInstalled = true;
          try {
            const RouteCarResultData = frida_java_bridge_default.use("com.autosdk.bussiness.navi.route.model.RouteCarResultData");
            const setFocusIndex = RouteCarResultData.setFocusIndex.overload("int");
            setFocusIndex.implementation = function(index) {
              const ret = setFocusIndex.call(this, index);
              enqueueTask("xingrui:RouteCarResultData.setFocusIndex", function() {
                sendCachedNativeRouteByIndex(index, "native-cache:RouteCarResultData.setFocusIndex[" + index + "]");
              });
              return ret;
            };
            log("RouteCarResultData focus export installed");
          } catch (e) {
            log("RouteCarResultData focus export hook failed: " + (e && e.stack ? e.stack : e));
          }
          try {
            const NaviPresenter = frida_java_bridge_default.use("com.autosdk.drive.navi.presenter.NaviPresenter");
            const suggestChangePathID = NaviPresenter.suggestChangePathID.overload("long");
            suggestChangePathID.implementation = function(pathId) {
              const ret = suggestChangePathID.call(this, pathId);
              enqueueTask("xingrui:NaviPresenter.suggestChangePathID", function() {
                sendCachedNativeRouteByPathId(pathId, "native-cache:NaviPresenter.suggestChangePathID[" + pathId + "]");
              });
              return ret;
            };
            log("NaviPresenter suggestChangePathID export installed");
          } catch (e) {
            log("NaviPresenter suggestChangePathID hook failed: " + (e && e.stack ? e.stack : e));
          }
          try {
            const NaviPresenter = frida_java_bridge_default.use("com.autosdk.drive.navi.presenter.NaviPresenter");
            const switchAlternateRoute = NaviPresenter.switchAlternateRoute.overload(
              "com.autosdk.bussiness.navi.route.model.RouteCarResultData",
              "int"
            );
            switchAlternateRoute.implementation = function(routeData, index) {
              const ret = switchAlternateRoute.call(this, routeData, index);
              enqueueTask("xingrui:NaviPresenter.switchAlternateRoute", function() {
                sendCachedNativeRouteByIndex(index, "native-cache:NaviPresenter.switchAlternateRoute[" + index + "]");
              });
              return ret;
            };
            log("NaviPresenter switchAlternateRoute export installed");
          } catch (e) {
            log("NaviPresenter switchAlternateRoute hook failed: " + (e && e.stack ? e.stack : e));
          }
        } catch (e) {
          keep.routeFocusExportInstalled = false;
          log("install route focus export failed: " + (e && e.stack ? e.stack : e));
        }
      }
      function installLocationSocketExport() {
        try {
          let closeSocket = function() {
            try {
              if (keep.locWriter !== null) {
                keep.locWriter.close();
              }
            } catch (e) {
            }
            try {
              if (keep.locSocket !== null) {
                keep.locSocket.close();
              }
            } catch (e) {
            }
            keep.locWriter = null;
            ...
            const now = Date.now();
            if (now - keep.locLastConnectAt < 1e3) {
              return false;
            }
            keep.locLastConnectAt = now;
            try {
              const socket = Socket.$new(LOC_SOCKET_HOST, LOC_SOCKET_PORT);
              socket.setTcpNoDelay(true);
              keep.locSocket = socket;
              keep.locWriter = BufferedWriter.$new(OutputStreamWriter.$new(socket.getOutputStream(), "UTF-8"));
              log("loc socket connected");
              sendHeartbeat(true);
              return true;
            } catch (e) {
              closeSocket();
              if (now - keep.locLastLogAt > 5e3) {
                keep.locLastLogAt = now;
                log("loc socket connect failed: " + e);
              }
              return false;
            }
          }, flushPendingSocketLines = function() {
            if (keep.socketFlushInProgress || keep.pendingSocketLines.length === 0) {
              return;
            }
            keep.socketFlushInProgress = true;
            if (!ensureSocket()) {
              keep.socketFlushInProgress = false;
              diag("socket unavailable for queued lines");
              return false;
            }
            try {
              while (keep.pendingSocketLines.length > 0) {
                const item = keep.pendingSocketLines.shift();
                keep.locWriter.write(item.line, 0, item.line.length);
              }
              keep.locWriter.flush();
              return true;
            } catch (e) {
              closeSocket();
              diag("queued socket send failed: " + e);
              return false;
            } finally {
              keep.socketFlushInProgress = false;
            }
          }, numberField = function(obj, name, fallback) {
            try {
              if (obj !== null && obj !== void 0 && obj[name] !== void 0) {
                return Number(obj[name].value);
              }
            } catch (e) {
            }
            return fallback;
          }, objectField = function(obj, name) {
            try {
              if (obj !== null && obj !== void 0 && obj[name] !== void 0) {
                return obj[name].value;
              }
            ...
            const now = Date.now();
            if (now - keep.locLastDiagAt > 5e3) {
              keep.locLastDiagAt = now;
              log("loc diag: " + message);
            }
          }, sendHeartbeat = function(forceHello) {
            if (typeof keep.sendSocketLine !== "function") {
              return;
            }
            const now = Date.now();
            if (!forceHello && now - (keep.lastHeartbeatAt || 0) < 3e3) {
              return;
            }
            keep.lastHeartbeatAt = now;
            keep.sendSocketLine((forceHello ? "HELLO" : "PING") + "|" + now + "|" + VEHICLE_PROFILE + "|" + PROVIDER + "|" + Process.id + "|" + HOOK_VERSION + "|" + MAP_PACKAGE + "\n", "heartbeat");
          }, queueCarLocation = function(lat, lon, course, speed, segmCur, linkCur, postCur, source) {
            if (source !== "drInfo") {
              return;
            }
            if (!isValidCoord(lat, lon)) {
              diag(source + " invalid coord lat=" + lat + ", lon=" + lon);
              return;
            }
            keep.lastPendingSeq++;
            keep.pendingCarLocation = {
              seq: keep.lastPendingSeq,
              time: Date.now(),
              lat,
              lon,
              course,
          ...
          }, flushQueuedCarLocation = function() {
            const pending = keep.pendingCarLocation;
            if (pending === null || pending.seq === keep.lastFlushedSeq) {
              return;
            }
            const now = Date.now();
            if (now - keep.locLastSentAt < 25) {
              return;
            }
            {
              keep.locLastSentAt = now;
              const line = "LOC|" + pending.time + "|" + pending.lat + "|" + pending.lon + "|" + pending.course + "|" + pending.speed + "|" + pending.segmCur + "|" + pending.linkCur + "|" + pending.postCur + "|1\n";
              if (!keep.sendSocketLine(line, pending.source + " loc")) {
                return;
              }
              flushPendingSocketLines();
              keep.lastFlushedSeq = pending.seq;
              if (now - keep.locLastLogAt > 5e3) {
                keep.locLastLogAt = now;
                log("loc sent " + pending.source + " lat=" + pending.lat + ", lon=" + pending.lon + ", dir=" + pending.course);
              }
            }
          }, sendDrInfo = function(drInfo) {
            try {
              if (drInfo === null || drInfo === void 0) {
                return;
              }
              let pos = objectField(drInfo, "drMatchPos");
              if (pos === null) {
                pos = objectField(drInfo, "drRawPos");
              }
              if (pos === null) {
                diag("drInfo pos null");
                return;
              }
              const lon = numberField(pos, "lon", 0) / 1e6;
              const lat = numberField(pos, "lat", 0) / 1e6;
              const course = numberField(drInfo, "drMatchAzi", numberField(drInfo, "drAzi", 0));
              const speed = numberField(drInfo, "spd", 0);
              queueCarLocation(lat, lon, course, speed, -1, -1, -1, "drInfo");
            } catch (e) {
              diag("send drInfo failed: " + e);
            }
          };
          if (keep.locationSocketExportInstalled) {
            return;
          }
          keep.locationSocketExportInstalled = true;
          const Socket = frida_java_bridge_default.use("java.net.Socket");
          const OutputStreamWriter = frida_java_bridge_default.use("java.io.OutputStreamWriter");
          const BufferedWriter = frida_java_bridge_default.use("java.io.BufferedWriter");
          keep.locSocket = null;
          keep.locWriter = null;
          keep.locLastConnectAt = 0;
          keep.locLastLogAt = 0;
          keep.locLastSentAt = 0;
          keep.locLastDiagAt = 0;
          keep.pendingCarLocation = null;
          keep.pendingSocketLines = [];
          keep.socketFlushInProgress = false;
          keep.lastPendingSeq = 0;
          keep.lastFlushedSeq = 0;
          keep.lastHeartbeatAt = 0;
          keep.sendSocketLine = function(line, label) {
            keep.pendingSocketLines.push({
              line,
              label,
              time: Date.now()
            });
            if (keep.pendingSocketLines.length > 100) {
              keep.pendingSocketLines.splice(0, keep.pendingSocketLines.length - 100);
            }
            return true;
          };
          try {
            const NmeaDrObserver = frida_java_bridge_default.use("com.autosdk.common.location.NmeaManager$1");
            const onDrInfoUpdate = NmeaDrObserver.onDrInfoUpdate.overload("com.autonavi.gbl.pos.model.DrInfo");
            onDrInfoUpdate.implementation = function(drInfo) {
              sendDrInfo(drInfo);
              return onDrInfoUpdate.call(this, drInfo);
            };
            log("NmeaManager DrInfo hook installed");
          } catch (e) {
            log("NmeaManager DrInfo hook failed: " + e);
          }
          setInterval(function() {
            runPendingTasks(8);
            sendHeartbeat(false);
            flushQueuedCarLocation();
            flushPendingSocketLines();
          }, 25);
          log("location socket export installed");
        } catch (e) {
          keep.locationSocketExportInstalled = false;
          log("install location socket export failed: " + e.stack);
        }
      }
      try {
        installRoutePointExportWhenReady(1);
      } catch (e) {
        log("installRoutePointExportWhenReady entry failed: " + e.stack);
      }
      try {
        installLocationSocketExport();
      } catch (e) {
        log("installLocationSocketExport entry failed: " + e.stack);
      }
      try {
        installGuidanceWidgetExport();
      } catch (e) {
        log("installGuidanceWidgetExport entry failed: " + e.stack);
      }
      try {
        installRouteFocusExport();
      } catch (e) {
        log("installRouteFocusExport entry failed: " + e.stack);
      }
      try {
        installPresenterGuidanceExport();
      } catch (e) {
        log("installPresenterGuidanceExport entry failed: " + e.stack);
      }
      try {
        installExternalInteractionProtocolExport();
      } catch (e) {
        log("installExternalInteractionProtocolExport entry failed: " + e.stack);
      }
      try {
        startNavigationStatePolling();
      } catch (e) {
        log("startNavigationStatePolling entry failed: " + e.stack);
      }
      log("route, location and guidance export mode active");
    });
  }
});
require_hook_geely_map_overlay_widget();
