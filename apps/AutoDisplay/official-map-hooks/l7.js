...
// hook_flyme_l7_map_route.js
var require_hook_flyme_l7_map_route = __commonJS({
  "hook_flyme_l7_map_route.js"() {
    init_node_globals();
    ...
    var TAG = "[AutoDisplayL7Hook]";
    var HOST = "127.0.0.1";
    var PORT = 23791;
    var PROTOCOL_VERSION = 1;
    var PROVIDER = "flyme_l7";
    var VEHICLE_PROFILE = "geely_l7";
    var MAP_PACKAGE = "com.flyme.auto.map";
    var HOOK_VERSION = "2026.07.08.l7-auto-heartbeat-ready";
    var COORDINATE_SYSTEM = "gcj02";
    var POINT_ORDER = "lon_lat";
    var POINT_UNIT = "degrees";
    var keep = {
      writer: null,
      socket: null,
      lastConnectAt: 0,
      lastSignature: "",
      lastRouteSentAt: 0,
      lastRouteStopPollAt: 0,
      lastLocSentAt: 0,
      lastLocLogAt: 0,
      lastLocDiagAt: 0,
      lastLocDiagSource: "",
      lastLocationLine: "",
      lastLocationReplayAt: 0,
      lastTrafficLightSignature: "",
      lastTrafficLightSentAt: 0,
      trafficLightCallCount: 0,
      trafficLightSentCount: 0,
      trafficLightClearCount: 0,
      lastTrafficLightLogDistance: -1,
      lastTrafficLightProbeLogAt: 0,
      lastTrafficLightProbeSignature: "",
      lastGuideSignature: "",
      lastGuideSentAt: 0,
      lastLaneSignature: "",
      lastLaneSentAt: 0,
      ...
      routeDirty: false,
      lastGuidanceRouteExportAt: 0,
      locationHookInstalled: false,
      pendingLines: [],
      flushing: false,
      pendingTasks: [],
    ...
    function logTrafficLightProbe(stage, details, force) {
      const now = Date.now();
      const signature = stage + "|" + details;
      const early = keep.trafficLightCallCount <= 30 || keep.trafficLightSentCount <= 10 || keep.trafficLightClearCount <= 10;
      if (force || early || signature !== keep.lastTrafficLightProbeSignature || now - keep.lastTrafficLightProbeLogAt >= 1e3) {
        keep.lastTrafficLightProbeSignature = signature;
        keep.lastTrafficLightProbeLogAt = now;
        log("TL-PROBE " + stage + " call#" + keep.trafficLightCallCount + " sent#" + keep.trafficLightSentCount + " clear#" + keep.trafficLightClearCount + " socket=" + (keep.writer ? 1 : 0) + " routeActive=" + (keep.routeActive ? 1 : 0) + " " + details);
      }
    }
    function closeSocket() {
      try {
        if (keep.writer) keep.writer.close();
      } catch (e) {
      }
      try {
        if (keep.socket) keep.socket.close();
      } catch (e) {
      }
      keep.writer = null;
      keep.socket = null;
    ...
    function ensureSocket() {
      if (keep.writer) return true;
      const now = Date.now();
      if (now - keep.lastConnectAt < 1e3) return false;
      keep.lastConnectAt = now;
      try {
        ...
        const socket = Socket.$new(HOST, PORT);
        socket.setTcpNoDelay(true);
        keep.socket = socket;
        keep.writer = BufferedWriter.$new(OutputStreamWriter.$new(socket.getOutputStream(), "UTF-8"));
    ...
    function sendLine(line) {
      keep.pendingLines.push(line);
      if (keep.pendingLines.length > 200) {
        keep.pendingLines.splice(0, keep.pendingLines.length - 200);
      }
    }
    function sendHeartbeat(forceHello) {
      const now = Date.now();
      if (!forceHello && now - keep.lastHeartbeatAt < 3e3) return;
      keep.lastHeartbeatAt = now;
      sendLine((forceHello ? "HELLO" : "PING") + "|" + now + "|" + VEHICLE_PROFILE + "|" + PROVIDER + "|" + Process.id + "|" + HOOK_VERSION + "|" + MAP_PACKAGE + "\n");
    }
    ...
    function runPendingTasks(maxTasks) {
      if (keep.processingTasks || keep.pendingTasks.length === 0) return;
    ...
    function flushPendingLines() {
      if (keep.flushing || keep.pendingLines.length === 0) return;
      keep.flushing = true;
      const batch = keep.pendingLines.splice(0, keep.pendingLines.length);
      try {
        if (!ensureSocket()) return;
        for (const line of batch) {
          keep.writer.write(line, 0, line.length);
        }
        keep.writer.flush();
      } catch (e) {
        keep.pendingLines = batch.concat(keep.pendingLines);
        if (keep.pendingLines.length > 200) {
          keep.pendingLines.splice(0, keep.pendingLines.length - 200);
        }
        closeSocket();
    ...
    function normalizeCoordinate(raw, axis) {
      const value = Number(raw);
      if (!Number.isFinite(value)) return 0;
      const limit = axis === "lat" ? 90 : 180;
      const abs2 = Math.abs(value);
      if (abs2 <= limit) return value;
      if (abs2 <= limit * 1e6) return value / 1e6;
      if (axis === "lon" && abs2 >= 252e6 && abs2 <= 504e6) return value / 36e5;
      if (axis === "lat" && abs2 >= 54e6 && abs2 <= 198e6) return value / 36e5;
      if (abs2 <= limit * 1e7) return value / 1e7;
      if (Math.abs(value / 36e5) <= limit) return value / 36e5;
      return value;
    }
    function isValidLatLon(lat, lon) {
      return Number.isFinite(lat) && Number.isFinite(lon) && lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180 && !(lat === 0 && lon === 0);
    }
    function sendLocation(lat, lon, bearing, speed, segmentIndex, linkIndex, pointIndex, source, force) {
      if (!isValidLatLon(lat, lon)) return false;
      const now = Date.now();
      if (!force && now - keep.lastLocSentAt < 25) return false;
      keep.lastLocSentAt = now;
      const line = "LOC|" + now + "|" + lat + "|" + lon + "|" + bearing + "|" + (Number.isFinite(speed) ? speed : 0) + "|" + (Number.isFinite(segmentIndex) ? segmentIndex : -1) + "|" + (Number.isFinite(linkIndex) ? linkIndex : -1) + "|" + (Number.isFinite(pointIndex) ? pointIndex : -1) + "|1|" + PROTOCOL_VERSION + "|" + PROVIDER + "|" + source + "|" + COORDINATE_SYSTEM + "\n";
      keep.lastLocationLine = line;
      sendLine(line);
      if (source !== keep.lastLocDiagSource || now - keep.lastLocDiagAt > 1e3) {
        keep.lastLocDiagAt = now;
        keep.lastLocDiagSource = source;
        log("LOC send source=" + source + " force=" + (force ? 1 : 0) + " routeActive=" + (keep.routeActive ? 1 : 0) + " lat=" + lat + " lon=" + lon + " bearing=" + bearing + " speed=" + (Number.isFinite(speed) ? speed : 0) + " seg=" + (Number.isFinite(segmentIndex) ? segmentIndex : -1) + " link=" + (Number.isFinite(linkIndex) ? linkIndex : -1) + " point=" + (Number.isFinite(pointIndex) ? pointIndex : -1));
      }
      return true;
    }
    function replayCachedLocationIfNeeded() {
      if (!keep.lastLocationLine || keep.writer || keep.pendingLines.length > 0) return;
      const now = Date.now();
      if (now - keep.lastLocationReplayAt < 1e3) return;
      keep.lastLocationReplayAt = now;
      sendLine(keep.lastLocationLine);
    }
    function listSize(list) {
      try {
        return list ? Number(list.size()) : 0;
      } catch (e) {
        return 0;
      }
    }
    ...
    function coordIntValue(point, name) {
      try {
        if (!keep.coord2DInt32) {
          keep.coord2DInt32 = frida_java_bridge_default.use("com.autonavi.gbl.common.model.Coord2DInt32");
        }
        const coord = frida_java_bridge_default.cast(point, keep.coord2DInt32);
        if (coord && coord[name] !== void 0) {
          const value = Number(coord[name].value);
          if (Number.isFinite(value)) return value;
        }
      } catch (e) {
      }
      try {
        const fields = point.getClass().getFields();
        for (let i = 0; i < fields.length; i++) {
          const field = fields[i];
          if (String(field.getName()) === name) {
            const value = Number(field.get(point));
            if (Number.isFinite(value)) return value;
          }
        }
      } catch (e) {
    ...
    function appendPoints(route, points) {
      const count = listSize(points);
      for (let i = 0; i < count; i++) {
        try {
          const p = points.get(i);
          if (!p) continue;
          const lon = normalizeCoordinate(coordIntValue(p, "lon"), "lon");
          const lat = normalizeCoordinate(coordIntValue(p, "lat"), "lat");
          if (isValidLatLon(lat, lon)) {
            route.points.push([lon, lat]);
          }
        } catch (e) {
        }
      }
    }
    function buildRoute(path, index, selected, PathInfo) {
      const p = frida_java_bridge_default.cast(path, PathInfo);
      const route = {
        index,
        selected,
        pathId: num(p.getPathID(), 0),
        pathIndex: num(p.getPathIndex(), index),
        naviId: "",
        length: num(p.getLength(), 0),
        travelTime: num(p.getTravelTime(), 0),
        trafficLightCount: num(p.getTrafficLightCount(), 0),
        tollCost: num(p.getTollCost(), 0),
        points: []
      };
      try {
        route.naviId = String(p.getNaviID() || "");
      } catch (e) {
      }
      const segCount = num(p.getSegmentCount(), 0);
      route.signature = [
        route.index,
        route.pathId,
        route.pathIndex,
      ...
      route.routeId = route.signature;
      route.distanceMeters = route.length;
      route.durationSeconds = route.travelTime;
      for (let s = 0; s < segCount; s++) {
        try {
          const seg = p.getSegmentInfo(s);
          if (!seg) continue;
          const linkCount = num(seg.getLinkCount(), 0);
          let before = route.points.length;
          for (let l = 0; l < linkCount; l++) {
            try {
              const link = seg.getLinkInfo(l);
              if (link) appendPoints(route, link.getPoints());
            } catch (e) {
            }
          }
          if (route.points.length === before) {
            try {
              appendPoints(route, seg.getPoints());
            } catch (e) {
            }
          }
        } catch (e) {
    ...
    function routePointSampleFingerprint(route) {
      try {
    ...
    function routeFingerprint(route) {
      ...
      return [
        ...
        route.tollCost,
        route.points.length,
        routePointSampleFingerprint(route)
      ].join(":");
    }
    function routePayload(source, navigating, selectIndex, routes) {
      const selected = routes && routes.length > 0 ? routes.find((r) => r.selected) || routes[Math.max(0, Math.min(selectIndex || 0, routes.length - 1))] : null;
      return {
        ...
        source,
        navigating,
        selectIndex: selected ? selected.index : Math.max(0, selectIndex || 0),
        routeCount: routes ? routes.length : 0,
        pointCount: selected ? selected.points.length : 0,
        routes: routes || []
      };
    }
    function logSparse(message) {
      const now = Date.now();
      if (now - keep.lastPollLogAt > 8e3) {
        keep.lastPollLogAt = now;
        log(message);
      }
    }
    function canExportRoute(source) {
      if (keep.writer) return true;
      const now = Date.now();
      if (now - keep.lastConnectAt < 5e3) return false;
      if (ensureSocket()) return true;
      logSparse(source + " skip route export: socket unavailable");
      return false;
    }
    function exportResult(result, source) {
      try {
        if (!result) {
          if (keep.routeActive) {
            sendStop(source + ":nullResult");
          }
          if (source.indexOf("poll") >= 0) {
            logSparse(source + " result=null");
          } else {
            log(source + " result=null");
          }
          return false;
        }
        if (!canExportRoute(source)) return false;
        const PathInfo = frida_java_bridge_default.use("com.autonavi.gbl.common.path.option.PathInfo");
        const paths = result.getPathResult();
        const count = listSize(paths);
        let focus = num(result.getFocusIndex(), 0);
        if (focus < 0 || focus >= count) focus = 0;
        const routes = [];
        for (let i = 0; i < count; i++) {
          try {
            const route = buildRoute(paths.get(i), i, i === focus, PathInfo);
            if (route.points.length > 1) routes.push(route);
          } catch (e) {
            log(source + " route[" + i + "] failed: " + e);
          }
        }
        if (routes.length <= 0) {
          log(source + " no usable routes count=" + count);
          return false;
        }
        const selected = routes.find((r) => r.selected) || routes[0];
        const signature = "route:" + routeFingerprint(selected);
        const now = Date.now();
        if (signature === keep.lastSignature && now - keep.lastRouteSentAt < 1e4) return true;
        keep.lastSignature = signature;
        keep.lastRouteSentAt = now;
        keep.routeActive = true;
        const payload = routePayload(source, true, selected.index, routes);
        sendLine("ROUTE|" + JSON.stringify(payload) + "\n");
        log(source + " sent routes=" + routes.length + " selected=" + selected.index + " points=" + selected.points.length);
        return true;
      } catch (e) {
    ...
    function exportCurrentRouteResult(source) {
      try {
        const Manager = frida_java_bridge_default.use("com.flyme.auto.map.amap.adapter.route.RouteRequestManager");
        const instance = Manager.INSTANCE.value;
        if (!instance) {
          log(source + " route manager instance=null");
          return false;
        }
        const result = instance.getRouteCarResultData();
        if (!result) {
          if (keep.routeActive) sendStop(source + ":nullResult");
          log(source + " current route result=null");
          return false;
        }
        return exportResult(result, source);
      } catch (e) {
        log(source + " export current route failed: " + e.stack);
        return false;
      }
    }
    function markRouteDirty(source) {
      keep.routeDirty = true;
      log(source + " route dirty");
    }
    function scheduleRouteExport(source) {
      markRouteDirty(source);
      enqueueTask(source + ":routeExport", function() {
        exportCurrentNavigationRoute(source);
      });
    }
    function exportCurrentNavigationRoute(source) {
      if (!keep.guidanceActive) {
        logSparse(source + " skip route export: not guiding");
        return false;
      }
      const now = Date.now();
      if (!keep.routeDirty && now - keep.lastGuidanceRouteExportAt < 500) return false;
      keep.lastGuidanceRouteExportAt = now;
      keep.routeDirty = false;
      return exportCurrentRouteResult(source);
    }
    function sendStop(source) {
      keep.lastSignature = "";
      keep.routeActive = false;
      keep.guidanceActive = false;
      keep.routeDirty = false;
      sendLine("ROUTE|" + JSON.stringify(routePayload(source, false, 0, [])) + "\n");
      sendTrafficLightClear(source);
      sendGuideClear(source);
      sendLaneClear(source);
      log(source + " sent stop");
    }
    function stringField(obj, name, fallback) {
      try {
        const value = objectField(obj, name);
        if (value === null || value === void 0) return fallback;
        const text = String(value).trim();
        if (text.length <= 0 || text === "null" || text === "undefined") return fallback;
        return text;
      } catch (e) {
        return fallback;
      }
    ...
    function readGuidePanelAt(panels, index) {
      try {
        if (!panels || index < 0 || index >= listSize(panels)) return null;
    ...
    function guidePanelScore(panel, preferred) {
      if (!panel) return -1;
      const maneuverInfo = objectField(panel, "maneuverInfo");
      const maneuverId = Math.floor(numberField(panel, "maneuverID", -1));
      const maneuverInfoId = Math.floor(numberField(maneuverInfo, "maneuverId", -1));
      ...
      if (maneuverId >= 0 || maneuverInfoId >= 0) score += 8;
      if (nextRoadName.length > 0) score += 6;
      if (segmentDistanceMeters >= 0) score += 4;
      if (segmentDistanceMeters > 0) score += 1;
      return score;
    }
    function readBestGuideContext(naviInfo) {
      ...
      let bestScore = -1;
      const tryPanel = (index, preferred) => {
        const panel = readGuidePanelAt(panels, index);
      ...
      for (let i = 0; i < panelCount; i++) {
        if (i !== flagIndex) tryPanel(i, false);
      }
      ...
      let naviScore = bestScore;
      if (routeDistanceMeters >= 0) naviScore += 1;
      if (routeTimeSeconds >= 0) naviScore += 1;
      return {
        naviInfo,
        panel: bestPanel,
        panelCount,
    ...
    function sendGuideFromNaviInfoList(naviInfoList, source) {
      try {
        ...
        const maneuverInfoId = Math.floor(numberField(maneuverInfo, "maneuverId", -1));
        if (maneuverId < 0 && maneuverInfoId >= 0) maneuverId = maneuverInfoId;
        const roundaboutExit = Math.max(0, Math.floor(numberField(maneuverInfo, "exitNum", numberField(naviInfo, "ringOutCnt", 0))));
        const segmentRemain = objectField(panel, "segmentRemain");
        const routeRemain = objectField(naviInfo, "routeRemain");
        const segmentDistanceMeters = Math.floor(numberField(segmentRemain, "dist", -1));
        const routeDistanceMeters = Math.floor(numberField(routeRemain, "dist", -1));
        ...
        const now = Date.now();
        if (signature === keep.lastGuideSignature && now - keep.lastGuideSentAt < 500) {
          return true;
        }
        ...
        log(source + " guide maneuver=" + maneuverId + " exit=" + roundaboutExit + " next=" + nextRoadName + " segmentDist=" + segmentDistanceMeters + " routeDist=" + routeDistanceMeters + " routeTime=" + routeTimeSeconds + " list=" + count + " panel=" + bestContext.panelIndex + "/" + bestContext.panelCount + " flag=" + bestContext.flagIndex + " score=" + bestContext.score);
        return true;
      } catch (e) {
        log(source + " guide export failed: " + e.stack);
        return false;
      }
    }
    function parseTbtField(text, name) {
    ...
    function parseTbtIntField(text, name, fallback) {
      const value = parseTbtField(text, name);
      if (value.length <= 0) return fallback;
      const number = Number(value);
      return Number.isFinite(number) ? Math.floor(number) : fallback;
    }
    function sendGuideFromTbtLog(text, source) {
      try {
        if (text.indexOf("GuidanceTbtData(") < 0) return false;
        const nextRoadName = parseTbtField(text, "firstRoadName");
        const maneuverId = parseTbtIntField(text, "firstManeuverIconId", -1);
        const segmentDistanceMeters = parseTbtIntField(text, "firstTurnDistanceNum", -1);
        const routeDistanceMeters = parseTbtIntField(text, "remainDistance", -1);
        const routeTimeSeconds = parseTbtIntField(text, "remainTime", -1);
        const roundaboutExit = Math.max(0, parseTbtIntField(text, "roundNum", 0));
        const currentRoad = parseTbtField(text, "currentRoad");
        if (maneuverId < 0 && nextRoadName.length <= 0 && segmentDistanceMeters < 0 && routeDistanceMeters < 0 && routeTimeSeconds < 0) {
          return false;
        }
        const signature = [
          "tbt",
        ...
        sendLine("GUIDE|" + JSON.stringify({
          ...
          routeDistanceMeters,
          routeTimeSeconds,
          routeRemainLightCount: -1,
          curRouteName: currentRoad,
          curSegIdx: -1,
          curLinkIdx: -1,
          curPointIdx: -1,
          fromTbtLog: true
        }) + "\n");
        log(source + " guide tbt maneuver=" + maneuverId + " next=" + nextRoadName + " segmentDist=" + segmentDistanceMeters + " routeDist=" + routeDistanceMeters + " routeTime=" + routeTimeSeconds + " current=" + currentRoad);
        return true;
      } catch (e) {
        log(source + " guide tbt parse failed: " + e.stack);
        return false;
      }
    }
    function parseGuidanceTbtLog(tag, message) {
      try {
        if (String(tag) !== "NaviObserverProxy") return;
        const text = String(message || "");
        if (text.indexOf("GuidanceTbtData(") < 0) return;
        sendGuideFromTbtLog(text, "l7:FLogUtils.GuidanceTbtData");
      } catch (e) {
        log("guidance tbt log parse failed: " + e.stack);
      }
    }
    function sendLaneFromInfo(laneInfo, source) {
      try {
        ...
        const signature = [
          frontLane.join(","),
          backLane.join(","),
          recommend.map((v) => v ? 1 : 0).join(","),
          extensionLane.join(",")
        ].join("|");
        const now = Date.now();
        if (signature === keep.lastLaneSignature && now - keep.lastLaneSentAt < 500) {
          return true;
        }
        ...
        log(source + " lane count=" + count + " front=" + frontLane.join(",") + " back=" + backLane.join(",") + " recommend=" + recommend.map((v) => v ? 1 : 0).join(","));
        return true;
      } catch (e) {
        log(source + " lane export failed: " + e.stack);
        return false;
      }
    }
    function mapL7LightDirection(rawDirection) {
    ...
    function mapL7GuidanceLogLightStatus(rawStatus) {
      const value = Math.floor(Number(rawStatus));
      if (value === 1) return 1;
      if (value === 2) return 4;
      if (value === 3 || value === 8) return 2;
      return -1;
    }
    function isUsableTrafficLightPayload(payload) {
      return payload && payload.status >= 0 && Number.isFinite(payload.seconds) && (payload.seconds > 0 || payload.status === 2);
    }
    function sendTrafficLightClear(source) {
      keep.lastTrafficLightSignature = "";
      keep.trafficLightClearCount += 1;
      logTrafficLightProbe("clear", "source=" + source, true);
      sendLine("TL|" + JSON.stringify({
        protocolVersion: PROTOCOL_VERSION,
      ...
      log(source + " traffic light clear");
    }
    function sendTrafficLightPayload(payload, source, force) {
      try {
        if (!payload || !isUsableTrafficLightPayload(payload)) {
          const invalidDetails = payload ? "source=" + source + " status=" + payload.status + " seconds=" + payload.seconds + " rawType=" + payload.rawLightType + " rawStatus=" + payload.rawLightStatus + " dir=" + payload.direction + " distance=" + payload.distanceMeters : "source=" + source + " payload=null";
          logTrafficLightProbe("invalid", invalidDetails, true);
          if (isAuthoritativeTrafficLightSource(source)) {
            sendTrafficLightClear(source + ":invalid");
          } else {
            log(source + " traffic light invalid ignored");
          }
          return false;
        }
        const now = Date.now();
        const signature = [
          ...
          payload.direction,
          payload.waitRound,
          payload.rawLightType,
          payload.rawLightStatus,
          payload.distanceMeters
        ].join(":");
        if (!force && signature === keep.lastTrafficLightSignature && now - keep.lastTrafficLightSentAt < 500) {
          logTrafficLightProbe("dedupe", "source=" + source + " signature=" + signature, false);
          return true;
        }
        keep.lastTrafficLightSignature = signature;
        keep.lastTrafficLightSentAt = now;
        keep.trafficLightSentCount += 1;
        sendLine("TL|" + JSON.stringify(payload) + "\n");
        logTrafficLightProbe("send", "source=" + source + " status=" + payload.status + " seconds=" + payload.seconds + " rawType=" + payload.rawLightType + " rawStatus=" + payload.rawLightStatus + " rawDir=" + payload.rawDirection + " dir=" + payload.direction + " wait=" + payload.waitRound + " distance=" + payload.distanceMeters + " id=" + payload.trafficLightId, true);
        log(source + " traffic light status=" + payload.status + " rawType=" + payload.rawLightType + " rawStatus=" + payload.rawLightStatus + " seconds=" + payload.seconds + " dir=" + payload.direction + " rawDir=" + payload.rawDirection + " wait=" + payload.waitRound + " distance=" + payload.distanceMeters);
        return true;
      } catch (e) {
        log(source + " traffic light send failed: " + e.stack);
        return false;
      }
    }
    function isAuthoritativeTrafficLightSource(source) {
      return source.indexOf("FLogUtils.GuidanceCountdown") >= 0;
    }
    function parseGuidanceTrafficLightLog(tag, message) {
      try {
        if (String(tag) !== "GuidanceNsHelper") return;
        const text = String(message || "");
        if (text.indexOf("onGuidanceTrafficLightTicker") >= 0) {
          const distanceMatch = /distance=(-?\d+)/.exec(text);
          if (distanceMatch) {
            keep.lastTrafficLightLogDistance = Number(distanceMatch[1]);
          }
          return;
        }
        if (text.indexOf("onGuidanceTrafficLightCountdown:") < 0) return;
        const statusMatch = /status=(-?\d+)/.exec(text);
        const waitMatch = /waitCount=(-?\d+)/.exec(text);
        const countMatch = /countDown=(-?\d+)/.exec(text);
        const directionMatch = /direction=(-?\d+)/.exec(text);
        if (!statusMatch || !countMatch || !directionMatch) {
          logTrafficLightProbe("log-parse-skip", "message=" + text.substring(0, 180), true);
          return;
        }
        keep.trafficLightCallCount += 1;
        const rawStatus = Number(statusMatch[1]);
        const status = mapL7GuidanceLogLightStatus(rawStatus);
        const seconds = Math.max(0, Number(countMatch[1]));
        const rawDirection = Number(directionMatch[1]);
        const payload = {
          protocolVersion: PROTOCOL_VERSION,
          messageType: "traffic_light",
          provider: PROVIDER,
          ...
          coordinateSystem: COORDINATE_SYSTEM,
          ts: Date.now(),
          source: "l7:FLogUtils.GuidanceCountdown",
          status,
          seconds,
          redSeconds: status === 1 ? seconds : 0,
          greenSeconds: status === 4 ? seconds : 0,
          direction: mapL7LightDirection(rawDirection),
          rawDirection,
          waitRound: waitMatch ? Math.max(0, Number(waitMatch[1])) : 0,
          distanceMeters: keep.lastTrafficLightLogDistance,
          rawLightType: -1,
          rawLightStatus: rawStatus
        };
        logTrafficLightProbe("log-parse", "status=" + status + " rawStatus=" + rawStatus + " seconds=" + seconds + " rawDir=" + rawDirection + " dir=" + payload.direction + " distance=" + payload.distanceMeters, false);
        sendTrafficLightPayload(payload, payload.source, false);
      } catch (e) {
        log("guidance traffic light log parse failed: " + e.stack);
      }
    }
    ...
    function sendLocInfo(locInfo, sourceOverride, force) {
      try {
        if (locInfo === null || locInfo === void 0) return false;
        const pos = objectField(locInfo, "stMatchRoadPos");
        const source = sourceOverride || "locInfo.matchRoad";
        let bearing = numberField(locInfo, "matchRoadCourse", 0);
        let speed = numberField(locInfo, "speedometer", numberField(locInfo, "speed", 0));
        let segmentIndex = numberField(locInfo, "segIdx", -1);
        let linkIndex = -1;
        let pointIndex = -1;
        const matchInfoList = objectField(locInfo, "matchInfo");
        try {
          if (matchInfoList && Number(matchInfoList.size()) > 0) {
            const match = matchInfoList.get(0);
            segmentIndex = numberField(match, "segmCur", segmentIndex);
            linkIndex = numberField(match, "linkCur", -1);
            pointIndex = numberField(match, "postCur", -1);
            bearing = numberField(match, "course", bearing);
          }
        } catch (e) {
        }
        if (pos === null) return false;
        const lon = normalizeCoordinate(numberField(pos, "lon", 0), "lon");
        const lat = normalizeCoordinate(numberField(pos, "lat", 0), "lat");
        return sendLocation(
          lat,
          lon,
          bearing,
        ...
        log("send locInfo failed: " + e);
        return false;
      }
    }
    function installLocationExport() {
      if (keep.locationHookInstalled) return true;
      try {
        const LocationInstrument = frida_java_bridge_default.use("com.autosdk.bussiness.location.LocationInstrument");
        const onLocInfoUpdate = LocationInstrument.onLocInfoUpdate.overload("com.autonavi.gbl.pos.model.LocInfo");
        onLocInfoUpdate.implementation = function(locInfo) {
          const ret = onLocInfoUpdate.call(this, locInfo);
          sendLocInfo(locInfo, "locInfo.matchRoad", false);
          return ret;
        };
        keep.locationHookInstalled = true;
        log("LocationInstrument LocInfo hook installed");
        return true;
      } catch (e) {
        logSparse("LocationInstrument LocInfo hook pending: " + e);
        return false;
      }
    }
    function pollRouteStopState(Manager) {
      if (!keep.routeActive) return;
      const now = Date.now();
      if (now - keep.lastRouteStopPollAt < 500) return;
      keep.lastRouteStopPollAt = now;
      try {
        const instance = Manager.INSTANCE.value;
        if (instance && !instance.getRouteCarResultData()) {
          sendStop("l7:routeStopPoll:nullResult");
        }
      } catch (e) {
        logSparse("route stop poll failed: " + e);
      }
    }
    function parseGuidanceLifecycleLog(tag, message) {
      try {
        const text = String(tag || "") + "|" + String(message || "");
        if (text.indexOf("stopGuidance") >= 0 || text.indexOf("stopNavi") >= 0 || text.indexOf("\u9000\u51FA\u5BFC\u822A") >= 0) {
          sendStop("l7:FLogUtils.guidanceStopLog");
        }
      } catch (e) {
        log("guidance lifecycle log parse failed: " + e.stack);
      }
    }
    function isInterestingGuidanceLog(tag, message) {
      try {
        const tagText = String(tag || "");
        const messageText = String(message || "");
        if (tagText === "NaviObserverProxy" && messageText.indexOf("GuidanceTbtData(") >= 0) return true;
        if (tagText === "GuidanceNsHelper" && (messageText.indexOf("onGuidanceTrafficLightTicker") >= 0 || messageText.indexOf("onGuidanceTrafficLightCountdown:") >= 0)) return true;
        if (messageText.indexOf("stopGuidance") >= 0 || messageText.indexOf("stopNavi") >= 0 || messageText.indexOf("\u9000\u51FA\u5BFC\u822A") >= 0) return true;
        return false;
      } catch (e) {
        return false;
      }
    }
    function installGuidanceStopHook() {
      if (keep.guidanceStopHookInstalled) return true;
      try {
        const GuidanceAdapter = frida_java_bridge_default.use("com.flyme.auto.map.base.interfaces.IGuidanceAdapter");
        const stopSimple = GuidanceAdapter.stopGuidance.overload("kotlin.jvm.functions.Function1");
        stopSimple.implementation = function(callback) {
          const ret = stopSimple.call(this, callback);
          enqueueTask("l7:IGuidanceAdapter.stopGuidance:stop", function() {
            sendStop("l7:IGuidanceAdapter.stopGuidance");
          });
          return ret;
        };
        const stopWithOptions = GuidanceAdapter.stopGuidance.overload(
          "boolean",
          "boolean",
          "kotlin.jvm.functions.Function1"
        );
        stopWithOptions.implementation = function(saveRoute, needToast, callback) {
          const ret = stopWithOptions.call(this, saveRoute, needToast, callback);
          enqueueTask("l7:IGuidanceAdapter.stopGuidance.options:stop", function() {
            sendStop("l7:IGuidanceAdapter.stopGuidance.options");
          });
          return ret;
        };
        keep.guidanceStopHookInstalled = true;
        log("guidance stop hook installed");
        return true;
      } catch (e) {
        logSparse("guidance stop hook pending: " + e);
        return false;
      }
    }
    function installGuidanceWidgetExport() {
      if (keep.guidanceWidgetHookInstalled) return true;
      try {
        const NaviObserverProxy = frida_java_bridge_default.use("com.flyme.auto.map.amap.adapter.guidance.proxy.NaviObserverProxy");
        const onUpdateNaviInfo = NaviObserverProxy.onUpdateNaviInfo.overload("java.util.ArrayList");
        onUpdateNaviInfo.implementation = function(naviInfoList) {
          const ret = onUpdateNaviInfo.call(this, naviInfoList);
          const retainedNaviInfoList = retainJavaObject(naviInfoList);
          enqueueTask("l7:NaviObserverProxy.onUpdateNaviInfo:guide", function() {
            sendGuideFromNaviInfoList(retainedNaviInfoList, "l7:NaviObserverProxy.onUpdateNaviInfo");
            const wasGuiding = keep.guidanceActive;
            keep.guidanceActive = true;
            if (!wasGuiding || !keep.routeActive) {
              scheduleRouteExport("l7:NaviObserverProxy.guidanceStart");
            }
          });
          return ret;
        };
        const onShowNaviLaneInfo = NaviObserverProxy.onShowNaviLaneInfo.overload("com.autonavi.gbl.guide.model.LaneInfo");
        onShowNaviLaneInfo.implementation = function(laneInfo) {
          const ret = onShowNaviLaneInfo.call(this, laneInfo);
          const retainedLaneInfo = retainJavaObject(laneInfo);
          enqueueTask("l7:NaviObserverProxy.onShowNaviLaneInfo:lane", function() {
            sendLaneFromInfo(retainedLaneInfo, "l7:NaviObserverProxy.onShowNaviLaneInfo");
          });
          return ret;
        };
        const onHideNaviLaneInfo = NaviObserverProxy.onHideNaviLaneInfo.overload();
        onHideNaviLaneInfo.implementation = function() {
          const ret = onHideNaviLaneInfo.call(this);
          enqueueTask("l7:NaviObserverProxy.onHideNaviLaneInfo:laneClear", function() {
            sendLaneClear("l7:NaviObserverProxy.onHideNaviLaneInfo");
          });
          return ret;
        };
        keep.guidanceWidgetHookInstalled = true;
        log("guidance widget hooks installed");
        return true;
      } catch (e) {
        logSparse("guidance widget hook pending: " + e);
        return false;
      }
    }
    function installRouteSwitchExport() {
      if (!keep.routeSwitchHookInstalled) {
        try {
          const AMapRouteAdapter = frida_java_bridge_default.use("com.flyme.auto.map.amap.adapter.AMapRouteAdapter");
          const switchRoute = AMapRouteAdapter.switchRoute.overload("int");
          switchRoute.implementation = function(index) {
            const ret = switchRoute.call(this, index);
            if (ret) {
              scheduleRouteExport("l7:AMapRouteAdapter.switchRoute[" + index + "]");
            }
            return ret;
          };
          keep.routeSwitchHookInstalled = true;
          log("route switch hook installed");
        } catch (e) {
          logSparse("route switch hook pending: " + e);
        }
      }
      if (!keep.routeFocusHookInstalled) {
        try {
          ...
          setFocusIndex.implementation = function(index) {
            setFocusIndex.call(this, index);
            scheduleRouteExport("l7:RouteCarResultData.setFocusIndex[" + index + "]");
          };
          keep.routeFocusHookInstalled = true;
          log("route focus hook installed");
        } catch (e) {
          logSparse("route focus hook pending: " + e);
        }
      }
      return !!(keep.routeSwitchHookInstalled && keep.routeFocusHookInstalled);
    }
    var l7RouteManagerClass = null;
    var l7HeartbeatLoopStarted = false;
    var l7InstallLoopStarted = false;
    var l7HooksInstalled = false;
    function getL7RouteManagerClass() {
      if (l7RouteManagerClass) return l7RouteManagerClass;
      try {
        l7RouteManagerClass = frida_java_bridge_default.use("com.flyme.auto.map.amap.adapter.route.RouteRequestManager");
        return l7RouteManagerClass;
      } catch (e) {
        logSparse("RouteRequestManager not ready: " + e);
        return null;
      }
    }
    function startL7HeartbeatLoop() {
      if (l7HeartbeatLoopStarted) return;
      l7HeartbeatLoopStarted = true;
      sendHeartbeat(true);
      flushPendingLines();
      setInterval(function() {
        try {
          sendHeartbeat(false);
          replayCachedLocationIfNeeded();
          flushPendingLines();
        } catch (e) {
          logSparse("heartbeat loop failed: " + e);
        }
      }, 25);
      log("heartbeat loop started");
    }
    function installL7HooksWhenReady() {
      const Manager = getL7RouteManagerClass();
      if (!Manager) return null;
      if (!keep.l7InstallReadyLogged) {
        keep.l7InstallReadyLogged = true;
        log("install start");
      }
      const locationOk = installLocationExport();
      const stopOk = installGuidanceStopHook();
      const widgetOk = installGuidanceWidgetExport();
      const routeSwitchOk = installRouteSwitchExport();
      if (!keep.routeHolderHookInstalled) {
        try {
          const Holder = frida_java_bridge_default.use("com.autosdk.bussiness.navi.route.RouteResultDataHolder");
          const setData = Holder.setRouteCarResultData.overload("com.autosdk.bussiness.navi.route.model.RouteCarResultData");
          setData.implementation = function(result) {
            const ret = setData.call(this, result);
            scheduleRouteExport("l7:holder.setRouteCarResultData");
            return ret;
          };
          keep.routeHolderHookInstalled = true;
          log("holder hooked");
        } catch (e) {
          logSparse("holder hook pending: " + e);
        }
      }
      if (!keep.routeClearHookInstalled) {
        try {
          const clear = Manager.clearRouteCarResultData.overload();
          clear.implementation = function() {
            const ret = clear.call(this);
            enqueueTask("l7:clearRouteCarResultData:stop", function() {
              sendStop("l7:clearRouteCarResultData");
            });
            return ret;
          };
          keep.routeClearHookInstalled = true;
          log("clear hooked");
        } catch (e) {
          logSparse("clear hook pending: " + e);
        }
      }
      if (!keep.trafficLightLogHookInstalled) {
        try {
          log("traffic light hook installing");
          const FLogUtils = frida_java_bridge_default.use("com.flyme.auto.map.base.utils.FLogUtils");
          const logi = FLogUtils.logi.overload("java.lang.String", "java.lang.String");
          logi.implementation = function(tag, message) {
            const ret = logi.call(this, tag, message);
            let tagText = "";
            let messageText = "";
            try {
              tagText = String(tag || "");
            ...
            if (isInterestingGuidanceLog(tagText, messageText)) {
              enqueueTask("l7:FLogUtils.logi:guidance", function() {
                parseGuidanceLifecycleLog(tagText, messageText);
                parseGuidanceTbtLog(tagText, messageText);
                parseGuidanceTrafficLightLog(tagText, messageText);
              });
            }
            return ret;
          };
          keep.trafficLightLogHookInstalled = true;
          log("traffic light guidance log hooked");
        } catch (e) {
          logSparse("traffic light guidance log hook pending: " + e);
        }
      }
      if (!l7HooksInstalled && locationOk && stopOk && widgetOk && routeSwitchOk && keep.routeHolderHookInstalled && keep.routeClearHookInstalled && keep.trafficLightLogHookInstalled) {
        l7HooksInstalled = true;
        log("install done");
      }
      return Manager;
    }
    function startL7InstallLoop() {
      if (l7InstallLoopStarted) return;
      l7InstallLoopStarted = true;
      setInterval(function() {
        try {
          const Manager = installL7HooksWhenReady();
          if (Manager) {
            pollRouteStopState(Manager);
          }
          runPendingTasks(8);
        } catch (e) {
          logSparse("install loop failed: " + e);
        }
      }, 25);
      log("install retry loop started");
    }
    function bootstrapL7Heartbeat() {
      try {
        frida_java_bridge_default.performNow(function() {
          log("heartbeat bootstrap start");
          startL7HeartbeatLoop();
          log("heartbeat bootstrap done");
        });
      } catch (e) {
        log("heartbeat performNow failed: " + (e && e.stack ? e.stack : e));
        setInterval(function() {
          if (l7HeartbeatLoopStarted) return;
          try {
            frida_java_bridge_default.performNow(function() {
              log("heartbeat retry bootstrap start");
              startL7HeartbeatLoop();
              log("heartbeat retry bootstrap done");
            });
          } catch (retryError) {
            logSparse("heartbeat performNow retry failed: " + retryError);
          }
        }, 1e3);
      }
    }
    function bootstrapL7InstallLoop() {
      try {
        frida_java_bridge_default.perform(function() {
          startL7InstallLoop();
        });
      } catch (e) {
        log("install perform failed: " + (e && e.stack ? e.stack : e));
        setInterval(function() {
          if (l7InstallLoopStarted) return;
          try {
            frida_java_bridge_default.perform(function() {
            ...
            logSparse("install perform retry failed: " + retryError);
          }
        }, 1e3);
      }
    }
    bootstrapL7Heartbeat();
    bootstrapL7InstallLoop();
  }
});
require_hook_flyme_l7_map_route();
